...
# Counter A → B
PB=$(curl -s -X POST "$API_BASE/proposals/$PA_ID/counter" "${AUTH_HEADERS[@]}" -d '{"price": 95000}')
...
# Counter B → C
PC=$(curl -s -X POST "$API_BASE/proposals/$PB_ID/counter" "${AUTH_HEADERS[@]}" -d '{"price": 92000}')

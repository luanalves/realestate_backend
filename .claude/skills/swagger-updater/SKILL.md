...
description: "Use when creating, updating, or removing Swagger/OpenAPI documentation for this project's REST API. Triggers: swagger, openapi, api docs, endpoint documentation, api_endpoint, thedevkitchen_api_endpoint, /api/docs, openapi.json, documenting endpoint, add endpoint swagger, remove endpoint swagger, update swagger, atualizar swagger, documentar endpoint, adicionar endpoint, remover endpoint swagger. Swagger is generated dynamically from the `thedevkitchen_api_endpoint` database table — never by editing static files."
---
...
All endpoints MUST comply with ADR-005 (`docs/adr/ADR-005-openapi-30-swagger-documentation.md`):
...
Use `description` only for: functional behavior and business rules, auth/session/company requirements, query/path parameter explanation, validation notes, error response notes.
...
- `development-best-practices` — naming/security/performance conventions applied to the same controllers
- `postman-collection-manager` — keep the Postman collection in sync with what's documented here (ADR-016)

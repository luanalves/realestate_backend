...
from odoo.addons.api_gateway.controllers.auth_controller import AuthController
import json
...
class TestAuthController(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.app = cls.env['oauth.application'].create({
    ...
    def test_token_endpoint_client_credentials(self):
        ...
        token = self.env['oauth.token'].create({
    ...
    def test_token_endpoint_missing_grant_type(self):
        ...
        app = self.env['oauth.application'].search([
    ...
    def test_refresh_endpoint(self):
        ...
        found_token = self.env['oauth.token'].search([
            ('refresh_token', '=', 'test_refresh_token_123'),
            ('revoked', '=', False),
        ], limit=1)

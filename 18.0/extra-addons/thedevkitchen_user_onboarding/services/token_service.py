...
class PasswordTokenService:
    ...
    def generate_token(self, user, token_type, company=None, created_by=None):
        ...
        token_record = self.env['thedevkitchen.password.token'].sudo().create(token_vals)
    ...
    def validate_token(self, raw_token):
        ...
        # Find token record
        token_record = self.env['thedevkitchen.password.token'].sudo().search([
            ('token', '=', token_hash),
        ], limit=1)
    ...
    def invalidate_previous_tokens(self, user_id, token_type):
        ...
        Args:
            ...
            token_type: 'invite' or 'reset'
        """
        tokens = self.env['thedevkitchen.password.token'].sudo().search([
            ('user_id', '=', user_id),
            ('token_type', '=', token_type),
            ('status', '=', 'pending'),
        ])

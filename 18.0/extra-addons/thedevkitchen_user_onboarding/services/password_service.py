...
class PasswordService:
    ...
    def set_password(
        self, raw_token, password, confirm_password, ip_address=None, user_agent=None
    ):
        """
    ...
    def forgot_password(self, email):
        ...
        if not user:
            ...
            _logger.info(
                f"Forgot password requested for non-existent/inactive email: {email}"
            )
            return
        ...
        raw_token, token_record = self.token_service.generate_token(
            user=user, token_type="reset", company=company
        )
    ...
    def _send_reset_email(self, user, raw_token):
        ...
        try:
            ...
            template = self.env.ref(
                "thedevkitchen_user_onboarding.email_template_password_reset"
            )
    ...
    def _invalidate_user_sessions(self, user_id):
        ...
        try:
            sessions = (
                self.env["thedevkitchen.api.session"]
                .sudo()
                .search(
                    [
                        ("user_id", "=", user_id),
            ...
            if sessions:
                ...
                _logger.info(
                    f"Invalidated {len(sessions)} active sessions for user ID {user_id}"
                )

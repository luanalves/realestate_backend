...
class TestInviteAuthorization(TransactionCase):
    ...
    def setUp(self):
        ...
        self.group_prospector = self.env.ref(
            "quicksol_estate.group_real_estate_prospector"
        )
        self.group_receptionist = self.env.ref(
            "quicksol_estate.group_real_estate_receptionist"
        )
        self.group_financial = self.env.ref(
            "quicksol_estate.group_real_estate_financial"
        )
        ...
        self.owner_user = self._create_test_user(
            "owner@test.com", "Owner User", [self.group_owner]
        )
        self.director_user = self._create_test_user(
            "director@test.com", "Director User", [self.group_director]
        )
        self.manager_user = self._create_test_user(
            "manager@test.com", "Manager User", [self.group_manager]
        )
        self.agent_user = self._create_test_user(
            "agent@test.com", "Agent User", [self.group_agent]
        )
        self.prospector_user = self._create_test_user(
            "prospector@test.com", "Prospector User", [self.group_prospector]
        )

        # Initialize InviteService
        from odoo.addons.thedevkitchen_user_onboarding.services.invite_service import (
            InviteService,
        )
        ...
        for profile in profiles:
            with self.subTest(profile=profile):
                with self.assertRaises(UserError):
                    self.invite_service.check_authorization(
                        self.prospector_user, profile
                    )
    ...
    @patch(
        "odoo.addons.thedevkitchen_user_onboarding.services.invite_service.InviteService.send_invite_email"
    )
    def test_email_template_rendering(self, mock_send_email):
        ...
        test_user = self._create_test_user(
            "invite@test.com", "Test Invite User", [self.group_agent]
        )
    ...
    def test_invite_email_context_variables(self):
        ...
        self.assertEqual(
            email_template.model, "res.users", "Template should target res.users model"
        )

        # Verify template contains required placeholders
        body_html = email_template.body_html
        self.assertIn(
            "invite_link", body_html, "Template should contain invite_link variable"
        )
        self.assertIn(
            "expires_hours", body_html, "Template should contain expires_hours variable"
        )

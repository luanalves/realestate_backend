...
class TestEmailLinkSettings(TransactionCase):
    ...
    def test_invite_ttl_zero_rejected(self):
        ...
        self.assertTrue(
            "must be between 1" in error_message or "positive" in error_message,
    ...
    def test_invite_ttl_above_maximum_rejected(self):
        ...
        error_message = str(context.exception).lower()
        self.assertTrue(
            "must be between 1" in error_message or "720" in error_message,
    ...
    def test_default_values_are_set(self):
        ...
        self.assertEqual(
            settings.invite_link_ttl_hours, 24, "Default invite TTL should be 24"
        )
        self.assertEqual(
            settings.reset_link_ttl_hours, 24, "Default reset TTL should be 24"
        )
        self.assertEqual(
            settings.max_resend_attempts, 5, "Default max resend should be 5"
        )
        self.assertEqual(
            settings.rate_limit_forgot_per_hour, 3, "Default rate limit should be 3"
        )
        self.assertEqual(
            settings.frontend_base_url, "http://localhost:3000", "Default frontend URL"
        )
    ...
    def test_get_settings_returns_singleton(self):
        ...
        self.assertEqual(
            settings1.id, settings2.id, "get_settings() should return singleton"
        )

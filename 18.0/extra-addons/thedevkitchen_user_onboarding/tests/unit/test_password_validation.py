...
class TestPasswordValidation(TransactionCase):
    ...
    @patch('odoo.addons.thedevkitchen_user_onboarding.services.token_service.PasswordTokenService.validate_token')
    ...
    def test_password_less_than_8_chars_rejected(self, mock_validate):
        ...
        self.assertTrue(
            'password must be at least' in error_message or 
            'minimum' in error_message or 
            '8' in error_message,
    ...
    def test_password_not_equal_confirm_rejected(self, mock_validate):
        ...
        error_message = str(context.exception).lower()
        self.assertTrue(
            'match' in error_message or 
            'mismatch' in error_message or
            'do not match' in error_message,

...
class InviteController(http.Controller):
    ...
    def invite_user(self, **kwargs):
        ...
        try:
            ...
            if not self._validate_email_format(email):
                return self._error_response(400, 'validation_error', f'Invalid email format: {email}')
            
            # Validate profile value
            valid_profiles = ['owner', 'director', 'manager', 'agent', 'prospector', 
            ...
            company = request.env['thedevkitchen.estate.company'].sudo().browse(int(company_id))
            ...
            if profile == 'portal':
                ...
                try:
                    ...
                    if 'already exists' in str(e) or 'already registered' in str(e):
                        ...
                        return self._error_response(409, 'conflict', str(e), {'field': field})
            ...
            response_data = {
                ...
                'invite_sent_at': token_record.create_date.isoformat() if token_record.create_date else None,
                'invite_expires_at': token_record.expires_at.isoformat() if token_record.expires_at else None,
            }
            ...
            if profile == 'portal' and tenant:
                ...
                response_data['tenant'] = {
                    ...
                    'phone': tenant.phone,
                    'birthdate': tenant.birthdate.isoformat() if tenant.birthdate else None,
                    'company_id': data['company_id'],
                }
            ...
            links = [
                ...
                {'href': f'/api/v1/users/{user.id}/resend-invite', 'rel': 'resend_invite', 'type': 'POST'},
                {'href': '/api/v1/users', 'rel': 'collection', 'type': 'GET'},
            ]
            
            if tenant:
                links.insert(1, {'href': f'/api/v1/tenants/{tenant.id}', 'rel': 'tenant', 'type': 'GET'})
    ...
    @http.route('/api/v1/users/<int:user_id>/resend-invite', type='http', auth='none', methods=['POST'], csrf=False, cors='*')
    @require_jwt
    @require_session
    @require_company
    def resend_invite(self, user_id, **kwargs):
        ...
        try:
            ...
            if not requester_id or not company_id:
                return self._error_response(401, 'ERR_UNAUTHORIZED', 'Missing session context')
            ...
            if not user:
                return self._error_response(404, 'ERR_NOT_FOUND', f'User with ID {user_id} not found in your company')
            ...
            if not user.signup_pending:
                return self._error_response(
                    ...
                    details={'user_id': user_id, 'suggestion': 'Use POST /api/v1/auth/forgot-password'}
                )
            ...
            # Generate new token
            settings = request.env['thedevkitchen.email.link.settings'].sudo().get_settings()
            raw_token, token_record = token_service.generate_token(
                user=user,
                token_type='invite',
            ...
            # Resend invite email
            try:
                invite_service.send_invite_email(user, raw_token, settings.invite_link_ttl_hours)
                email_status = 'sent'
            except Exception as email_error:
                _logger.error(f"Failed to send resend invite email to {user.email}: {email_error}")
                email_status = 'failed'
            
            _logger.info(f"Invite resent to user {user.id} ({user.email}) by {requester.login}")
            ...
            return self._error_response(500, 'ERR_INTERNAL_SERVER_ERROR', 'An unexpected error occurred')

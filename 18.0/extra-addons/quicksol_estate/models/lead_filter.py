...
class RealEstateLeadFilter(models.Model):
    ...
    filter_domain = fields.Text(
        "Filter Criteria", required=True, help="JSON-encoded filter parameters"
    )
        ...
        for record in self:
            ...
            if existing:
                raise ValidationError(
                    _("You already have a saved filter with this name")
                )
    ...
    def apply_filter(self):
        ...
        if params.get("property_type_id"):
            domain.append(
                ("property_type_interest", "=", int(params["property_type_id"]))
            )

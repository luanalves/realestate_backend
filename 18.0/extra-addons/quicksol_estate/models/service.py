...
class RealEstateService(models.Model):
    ...
    def _cron_recompute_pendency(self):
        ...
        active_services = self.search(
            [("active", "=", True), ("stage", "not in", list(TERMINAL_STAGES))]
        )
        _logger.info(
            "Feature 015 cron: recomputing pendency for %d services",
            len(active_services),
        )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "/") == "/":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("real.estate.service") or "/"
                )

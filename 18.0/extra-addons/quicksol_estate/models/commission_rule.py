...
class RealEstateCommissionRule(models.Model):
        ...
        for rule in self:
            if rule.structure_type == "fixed":
                if rule.fixed_amount < 0:
                    raise ValidationError(
                        _("Fixed commission amount cannot be negative. " "Got: %.2f")
                        % rule.fixed_amount
                    )
    ...
    def _check_value_range(self):
        ...
        for rule in self:
            if rule.min_value > rule.max_value:
                raise ValidationError(
                    _(
                        "Minimum value (%.2f) cannot be greater than maximum value (%.2f)"
                    )
    ...
    def write(self, vals):
        ...
        if "company_id" in vals and any(
            rule.company_id.id != vals["company_id"] for rule in self
        ):
            raise ValidationError(
                _(
                    "Cannot change company of an existing commission rule. "
        ...
        for rule in self:
            if rule.structure_type == "percentage":
                name = _("%(percentage).2f%% commission - %(type)s") % {
                    ...
                    "type": dict(rule._fields["transaction_type"].selection)[
    ...
    def calculate_split_commission(
        self, property_id, transaction_amount, transaction_type="sale"
    ):
        ...
        if not property_id or not property_id.agent_id:
            raise ValidationError(
                _("Property must have an assigned agent to calculate commission.")
            )

        # Check if split applies (prospector_id exists and differs from agent_id)
        if (
            not property_id.prospector_id
            or property_id.prospector_id == property_id.agent_id
        ):
        ...
        if not 0.0 <= split_percentage <= 1.0:
            raise ValidationError(
                _(
                    "Invalid prospector commission split percentage: %(percentage).2f%%. "

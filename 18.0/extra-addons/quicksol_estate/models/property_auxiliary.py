...
class PropertyKey(models.Model):
    ...
    property_id = fields.Many2one(
        "real.estate.property", string="Property", ondelete="cascade", required=True
    )
...
class PropertyCommission(models.Model):
    ...
    @api.depends(
        "value", "commission_type", "property_id.price", "property_id.rent_price"
    )
            ...
            if commission.commission_type == "fixed":
                ...
                if commission.applies_to == "sale":
                    commission.commission_amount = (
                        (commission.property_id.price or 0) * commission.value / 100
                    )
                elif commission.applies_to == "rent":
                    commission.commission_amount = (
                        (commission.property_id.rent_price or 0)
                        * commission.value
                        / 100
                    )
    ...
    def _check_value(self):
        for commission in self:
            if commission.commission_type == "percentage" and (
                commission.value < 0 or commission.value > 100
            ):

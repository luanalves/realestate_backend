...
class CommissionSplitObserver(models.AbstractModel):
    ...
    def handle(self, event_name, data, **kwargs):
        ...
        if not data or not isinstance(data, dict):
            _logger.warning(
                "CommissionSplitObserver: Invalid sale data received (expected dict)"
            )
            return
        ...
        try:
            split_result = commission_rule.calculate_split_commission(
                property_obj, sale.sale_price, transaction_type="sale"
            )
        except Exception as e:
            _logger.error(
                "Sale ID %d: Error calculating commission split: %s", sale.id, str(e)
            )
            return
        ...
        CommissionTransaction.create(
            {
                ...
                "company_id": (
                    property_obj.company_ids[0].id
                    if property_obj.company_ids
                    else False
                ),

...
class UserCompanyValidatorObserver(models.AbstractModel):
    ...
    def handle(self, event_name, data):
        ...
        if not is_owner or is_system:
            _logger.debug(
                "UserCompanyValidator: Skipping (user is not Owner or is System Admin)"
            )
            return
        ...
        if not company_ids_in_vals:
            _logger.debug("UserCompanyValidator: No company_ids in vals, skipping")
            return
        ...
        if unauthorized_companies:
            unauthorized_names = (
                ...
                .browse(list(unauthorized_companies))
                .mapped("name")
            )
            raise ValidationError(
                _("You cannot assign users to companies you don't have access to: %s")
                % ", ".join(unauthorized_names)
            )

        _logger.info(
            f"UserCompanyValidator: Validation passed for user {current_user.name}"
        )

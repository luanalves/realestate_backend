...
class SecurityGroupAuditObserver(AbstractObserver):
    ...
    def handle(self, event_name, data):
        ...
        if not user:
            _logger.warning(
                "SecurityGroupAuditObserver: No user provided in event data"
            )
        ...
        changed_by_info = (
            f"by {changed_by.name} (ID: {changed_by.id})" if changed_by else "by system"
        )
    ...
    def _register_hook(self):
        ...
        _logger.info(
            "SecurityGroupAuditObserver registered for user.groups_changed events"
        )

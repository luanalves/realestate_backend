...
class AgentPropertyAssignment(models.Model):
    ...
    assignment_date = fields.Date(
        string="Assignment Date", default=fields.Date.today, required=True
    )
            ...
            if assignment.active:
                ...
                if duplicate:
                    raise ValidationError(
                        _("Agent %s is already assigned to property %s")
    ...
    def _check_company_match(self):
        ...
        for assignment in self:
            ...
            if assignment.company_id != assignment.agent_id.company_id:
                raise ValidationError(_("Assignment company must match agent company"))

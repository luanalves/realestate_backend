...
class ResUsers(models.Model):
    ...
    def _compute_owner_companies(self):
        ...
        owner_group = self.env.ref(
            "quicksol_estate.group_real_estate_owner", raise_if_not_found=False
        )
        for user in self:
            ...
            if user.cpf:
                ...
                if not cpf_validator.validate(cpf_clean):
                    raise ValidationError(
                        f"CPF inválido: {user.cpf}. Digite um CPF válido com 11 dígitos."
                    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            self.env["quicksol.event.bus"].emit(
                "user.before_create", {"vals": vals, "user_id": self.env.uid}
            )
    ...
    def write(self, vals):
        ...
        self.env["quicksol.event.bus"].emit(
            "user.before_write",
            {"vals": vals, "user_id": self.env.uid, "target_user_ids": self.ids},
        )
        ...
        self.env["quicksol.event.bus"].emit(
            "user.updated",
            {"user_ids": self.ids, "vals": vals, "updated_by": self.env.uid},
        )

...
class RealEstateLead(models.Model):
    ...
    phone = fields.Char("Phone", size=20, tracking=True, help="Primary phone number")

    email = fields.Char("Email", size=120, tracking=True, help="Email address")
    ...
    bedrooms_needed = fields.Integer(
        "Bedrooms Needed", tracking=True, help="Desired number of bedrooms"
    )

    min_area = fields.Float(
        "Minimum Area (m²)", digits=(10, 2), tracking=True, help="Minimum area in m²"
    )

    max_area = fields.Float(
        "Maximum Area (m²)", digits=(10, 2), tracking=True, help="Maximum area in m²"
    )
    ...
    first_contact_date = fields.Date(
        "First Contact Date", tracking=True, help="Date of first contact with client"
    )
    ...
    lost_date = fields.Date(
        "Lost Date", tracking=True, help="Date lead was marked as lost"
    )

    lost_reason = fields.Text(
        "Lost Reason", tracking=True, help="Reason why lead was lost"
    )
        ...
        for record in self:
            if record.agent_id and record.company_id:
                if record.agent_id.company_id != record.company_id:
                    raise ValidationError("Agent must belong to the lead's company.")
    ...
    def action_convert_to_sale(self, property_id):
        ...
        if property_obj.company_id != self.company_id:
            raise ValidationError(
                _("Agent does not have access to the selected property.")
            )
        ...
        try:
            ...
            self.message_post(
                body=_(
                    f"Lead converted to sale. Property: {property_obj.name}, Sale ID: {sale.id}"
                ),
                subtype_xmlid="mail.mt_note",
            )

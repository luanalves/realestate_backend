...
class RealEstateProposal(models.Model):
    ...
    def _compute_name(self):
        for rec in self:
            rec.name = (
                f"{rec.proposal_code} - {rec.property_id.name}"
                if rec.property_id
                else rec.proposal_code
            )
    ...
    def _check_valid_until_bounds(self):
        for rec in self:
            ...
            if rec.valid_until > create_dt + timedelta(days=90):
                raise ValidationError(
                    _("Validity date must be at most 90 days from the creation date.")
                )
    ...
    def _check_document_format(self):
        ...
        for rec in self:
            ...
            if vat and not validate_document(vat):
                raise ValidationError(
                    _('Client document "%s" is not a valid CPF or CNPJ.', vat)
                )
    ...
    def _check_counter_consistency(self):
        for rec in self:
            ...
            if parent.company_id != rec.company_id:
                raise ValidationError(
                    _("Counter-proposal must belong to the same company as its parent.")
                )
    ...
    def _check_one_active_in_chain(self):
        ...
        for rec in self:
            ...
            if rec.parent_proposal_id:
                ...
                if siblings:
                    raise ValidationError(
                        _("Another counter-proposal in this chain is already active.")
                    )
            else:
        ...
        for vals in vals_list:
            ...
            try:
                ...
                if (
                    ...
                    or "LockNotAvailable" in type(e).__name__
                ):
                    raise UserError(
                        _(
            ...
            if vals.get("proposal_code", "/") == "/":
                vals["proposal_code"] = (
                    self.env["ir.sequence"].next_by_code("real.estate.proposal") or "/"
                )
    ...
    def _validate_valid_until(self, valid_until_val):
        ...
        if valid_until_val > today + timedelta(days=90):
            raise ValidationError(
                _("Validity date must be at most 90 days from today.")
            )
            ...
            comp.write(
                {
                    "state": "cancelled",
                    "active": False,
                    "cancellation_reason": _("Superseded by accepted proposal %s")
                    % self.proposal_code,
        ...
        self._emit_event(
            "proposal.accepted",
            {
                ...
                "recipient_partner_ids": [
                    self.partner_id.id,
                    self.agent_id.user_id.partner_id.id,
                ],
                "hateoas_links": [
                    {
                        "rel": "create-contract",
    ...
    def action_counter(self, vals):
        ...
        if "proposal_value" in vals and vals["proposal_value"] == self.proposal_value:
            raise UserError(
                _("Counter-proposal value must differ from the parent proposal value.")
            )
            ...
            try:
                self.message_post(
                    body=_(
                        ...
                        "Please send manually if needed.",
                        event_type,
                    ),

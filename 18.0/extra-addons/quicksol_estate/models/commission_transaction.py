...
class RealEstateCommissionTransaction(models.Model):
    ...
    payment_date = fields.Date(
        "Payment Date", tracking=True, help="Date when commission was paid to agent"
    )

    payment_notes = fields.Text(
        "Payment Notes", help="Additional notes about commission payment"
    )
    ...
    display_name = fields.Char(
        "Display Name", compute="_compute_display_name", store=False
    )
        ...
        for transaction in self:
            if transaction.rule_snapshot:
                try:
                    ...
                    transaction.rule_structure_type = snapshot.get(
    ...
    def _compute_display_name(self):
        ...
        for transaction in self:
            if transaction.agent_id and transaction.commission_amount:
                transaction.display_name = _(
                    "R$ %(amount).2f - %(agent)s (%(date)s)"
                ) % {
                ...
                transaction.display_name = (
                    _("Commission Transaction #%s") % transaction.id
                )
        ...
        for transaction in self:
            if transaction.transaction_amount > 0:
                commission_percentage = (
                    transaction.commission_amount / transaction.transaction_amount
                ) * 100
                if commission_percentage > 50:
                    raise ValidationError(
                        _(
                            "Commission (%.2f%%) exceeds 50%% of transaction amount. "
    ...
    def write(self, vals):
        ...
        for field in immutable_fields:
            if field in vals:
                raise UserError(
                    _(
                        "Cannot modify %(field)s after commission transaction is created. "
        ...
        if "payment_status" in vals and not self.env.context.get("tracking_disable"):
            for transaction in self:
                transaction.message_post(
                    ...
                    % dict(transaction._fields["payment_status"].selection)[
    ...
    def unlink(self):
        ...
        raise UserError(
            _(
                "Commission transactions cannot be deleted for audit purposes. "
    ...
    def action_mark_paid(self):
        ...
        self.message_post(
            ...
            % {"amount": self.commission_amount, "date": self.payment_date}
        )
    ...
    def action_cancel(self):
        ...
        if self.payment_status == "paid":
            raise UserError(
                _(
                    "Cannot cancel a commission that has been paid. "
        ...
        self.message_post(body=_("Commission transaction cancelled"))

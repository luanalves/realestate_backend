...
class Profile(models.Model):
    ...
    # ===== Cadastral Fields =====
    name = fields.Char(
        "Full Name", size=200, required=True, tracking=True, help="Full legal name"
    )
    ...
    email = fields.Char(
        "Email", size=100, required=True, tracking=True, help="Contact email address"
    )
    phone = fields.Char("Phone", size=20, help="Phone number (landline or mobile)")
    mobile = fields.Char("Mobile", size=20, help="Mobile phone number")
    ...
    hire_date = fields.Date(
        "Hire Date", help="Employment hire date (relevant for internal profiles)"
    )
    profile_picture = fields.Binary("Profile Picture", help="Profile photo")

    # ===== Soft Delete (ADR-015) =====
    active = fields.Boolean(
        "Active", default=True, help="Soft delete flag. False = profile is deactivated."
    )
    deactivation_date = fields.Datetime(
        "Deactivation Date", help="Timestamp when profile was deactivated"
    )
    deactivation_reason = fields.Text(
        "Deactivation Reason", help="Why this profile was deactivated"
    )
    ...
    def _compute_document_normalized(self):
        ...
        for record in self:
            if record.document:
                record.document_normalized = validators.normalize_document(
                    record.document
                )
            else:

...
class EventBus(models.AbstractModel):
    ...
    def _emit_sync(self, event_name, data):
        ...
        for model_name in self.env.registry.models.keys():
            try:
                ...
                if hasattr(model, "_inherit") and "quicksol.abstract.observer" in (
                    model._inherit
        ...
        for observer_model_name in observer_models:
            try:
                ...
                if observer.can_handle(event_name):
                    ...
                    _logger.debug(
                        f"Observer {observer_model_name} handled event '{event_name}'"
                    )
            except Exception as exc:
                _logger.error(
                    f"Observer {observer_model_name} failed to handle '{event_name}': {exc}",
                    exc_info=True,
                )
                if event_name.startswith("before_"):
                    raise
    ...
    def _emit_async(self, event_name, data):
        ...
        try:
            self.ASYNC_EVENTS[event_name]
            return self._emit_sync(event_name, data)

        except Exception as exc:
            _logger.error(
                f"Failed to emit async event '{event_name}', falling back to sync: {exc}",
                exc_info=True,
            )
            return self._emit_sync(event_name, data)

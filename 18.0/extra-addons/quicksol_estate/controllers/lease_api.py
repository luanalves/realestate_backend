...
class LeaseApiController(http.Controller):
    ...
    def _serialize_lease(self, lease):
        ...
        relations = {
            "property": (
                f"/api/v1/properties/{lease.property_id.id}"
                if lease.property_id
                else None
            ),
            "profile": (
                f"/api/v1/profiles/{lease.profile_id.id}" if lease.profile_id else None
            ),
        }
        ...
        return {
            ...
            "active": lease.active,
            "termination_date": (
                str(lease.termination_date) if lease.termination_date else None
            ),
    ...
    def list_leases(self, page=1, page_size=20, **kwargs):
        ...
        try:
            ...
            leases = Lease.search(
                domain, limit=page_size, offset=offset, order="start_date desc"
            )

            items = [self._serialize_lease(lease) for lease in leases]
    ...
    def create_lease(self, **kwargs):
        ...
        try:
            ...
            is_valid, errors = SchemaValidator.validate_request(
                data, SchemaValidator.LEASE_CREATE_SCHEMA
            )
            if not is_valid:
                resp, code = util_error(
            ...
            if end_date <= start_date:
                resp, code = util_error(
                    "End date must be after start date", status_code=400
                )
                return request.make_json_response(resp, status=code)
            ...
            links = build_hateoas_links(
                "/api/v1/leases",
                lease.id,
                {"renew": "/renew", "terminate": "/terminate"},
            )
            ...
            lease_data["renewal_history"] = [
                {
                    "id": rh.id,
                    "previous_end_date": (
                        str(rh.previous_end_date) if rh.previous_end_date else None
                    ),
    ...
    def update_lease(self, lease_id, **kwargs):
        ...
        try:
            ...
            if (
                lease.status in ("terminated", "expired")
            ...
            # Validate schema
            is_valid, errors = SchemaValidator.validate_request(
                data, SchemaValidator.LEASE_UPDATE_SCHEMA
            )
            if not is_valid:
                resp, code = util_error(
                    "Validation failed", errors={"validation": errors}, status_code=400
    ...
    @http.route(
        "/api/v1/leases/<int:lease_id>/renew",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def renew_lease(self, lease_id, **kwargs):
        ...
        try:
            ...
            # Validate schema
            is_valid, errors = SchemaValidator.validate_request(
                data, SchemaValidator.LEASE_RENEW_SCHEMA
            )
            if not is_valid:
                resp, code = util_error(
                    "Validation failed", errors={"validation": errors}, status_code=400
    ...
    @http.route(
        "/api/v1/leases/<int:lease_id>/terminate",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def terminate_lease(self, lease_id, **kwargs):
        ...
        try:
            ...
            # Validate schema
            is_valid, errors = SchemaValidator.validate_request(
                data, SchemaValidator.LEASE_TERMINATE_SCHEMA
            )
            if not is_valid:
                resp, code = util_error(
                    "Validation failed", errors={"validation": errors}, status_code=400
            ...
            try:
                datetime.strptime(data["termination_date"], "%Y-%m-%d").date()
            except ValueError:
                return error_response(400, "Invalid date format. Use YYYY-MM-DD")
            ...
            _logger.error(
                f"Error terminating lease {lease_id}: {str(e)}", exc_info=True
            )
            return error_response(500, "Internal server error")

...
def _serialize_service(service, include_messages=False):
    ...
    data = {
        ...
        "properties": [{"id": p.id, "name": p.name} for p in service.property_ids],
        "last_activity_date": (
            service.last_activity_date.isoformat()
            if service.last_activity_date
            else None
        ),
...
class ServiceController(http.Controller):
        ...
        try:
            ...
            if divergence:
                response_data["_warnings"] = [
                    {"code": "PARTNER_DEDUP_DIVERGENCE", "message": divergence}
                ]
    ...
    def get_service_summary(self, **kwargs):
        try:
            ...
            summary = pipeline_svc.compute_summary(
                sudo_env, company_id=company_id, agent_id=agent_id
            )
            data = {
                "total": summary["total"],
    ...
    def list_services(self, **kwargs):
        try:
            ...
            ordering = ORDERING_MAP.get(
                params.get("ordering", DEFAULT_ORDERING), ORDERING_MAP[DEFAULT_ORDERING]
            )
            ...
            try:
                ...
                per_page = min(
                    MAX_PAGE_SIZE,
                    max(1, int(params.get("per_page", DEFAULT_PAGE_SIZE))),
                )
            ...
            services = Service.search(
                full_domain, order=ordering, limit=per_page, offset=offset
            )
            ...
            links = [
                {
                    "rel": "self",
                    "href": f"{base_url}?page={page}&per_page={per_page}",
                    "method": "GET",
                },
            ]
            if page > 1:
                links.append(
                    {
                        "rel": "prev",
                        "href": f"{base_url}?page={page - 1}&per_page={per_page}",
                        "method": "GET",
                    }
                )
            if page < total_pages:
    ...
    # ---------------------------------------------------------------------- #
    @http.route(
        "/api/v1/services/<int:service_id>",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    def get_service(self, service_id, **kwargs):
        try:
            ...
            if (
                not service.exists()
                or service.company_id.id not in request.user_company_ids
            ):
    ...
    def update_service(self, service_id, **kwargs):
        try:
            ...
            if not _is_manager_or_owner(env) and service.agent_id.id != env.user.id:
                return error_response(
                    "FORBIDDEN", "Agents can only update their own services.", 403
                )
    ...
    def delete_service(self, service_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can delete services.", 403
                )
    ...
    # ---------------------------------------------------------------------- #
    @http.route(
        "/api/v1/services/<int:service_id>/stage",
        type="http",
        auth="none",
        methods=["PATCH"],
        csrf=False,
    ...
    def change_service_stage(self, service_id, **kwargs):
        try:
            ...
            if not _is_manager_or_owner(env) and service.agent_id.id != env.user.id:
                return error_response(
                    "FORBIDDEN",
                    "Agents can only change stage of their own services.",
                    403,
                )
            ...
            _logger.exception(
                "Unexpected error in change_service_stage id=%d", service_id
            )
            return error_response("INTERNAL_ERROR", "Internal server error.", 500)
    ...
    # ---------------------------------------------------------------------- #
    @http.route(
        "/api/v1/services/<int:service_id>/reassign",
        type="http",
        auth="none",
        methods=["PATCH"],
        csrf=False,
    ...
    def reassign_service(self, service_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can reassign services.", 403
                )

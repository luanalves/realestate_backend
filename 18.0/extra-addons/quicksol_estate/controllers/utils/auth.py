...
def require_jwt(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if not jwt_secret:
                _logger.critical(
                    "No JWT secret configured (oauth_jwt_secret or admin_passwd required)"
                )
            ...
            app = app_model.search(
                [("client_id", "=", payload.get("client_id"))], limit=1
            )

            if not app:
                client_id = payload.get("client_id", "")
                _logger.warning(
                    "JWT valid but application not found: %s", repr(client_id)[:100]
                )
        ...
        try:
            ...
            token = token_model.search(
                [("access_token", "=", access_token), ("revoked", "=", False)], limit=1
            )
            ...
            request.jwt_payload = {
                ...
                "user_id": (
                    token.application_id.create_uid.id

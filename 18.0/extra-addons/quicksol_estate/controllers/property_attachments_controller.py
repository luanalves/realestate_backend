...
def _get_max_upload_bytes():
    ...
    try:
        return int(
            IrConfig.get_param(CONFIG_PARAM_MAX_SIZE, default=DEFAULT_MAX_FILE_BYTES)
        )
...
def _get_max_images_per_property():
    try:
        val = (
            ...
            .sudo()
            .get_param(CONFIG_PARAM_MAX_IMAGES, default=50)
        )
        return max(1, int(val))
    except (ValueError, TypeError):
        _logger.warning(
            "Invalid value for %s, using default 50", CONFIG_PARAM_MAX_IMAGES
        )
        return 50
...
def _get_max_documents_per_property():
    try:
        val = (
            request.env["ir.config_parameter"]
            .sudo()
            .get_param(CONFIG_PARAM_MAX_DOCUMENTS, default=20)
        )
        return max(1, int(val))
    except (ValueError, TypeError):
        _logger.warning(
            "Invalid value for %s, using default 20", CONFIG_PARAM_MAX_DOCUMENTS
        )
        return 20
...
def _serialize_attachment(att, property_id, include_self_link=False):
    ...
    uploaded_at = (
        att.create_date.strftime("%Y-%m-%dT%H:%M:%SZ") if att.create_date else None
    )
    ...
    def upload_attachment(self, property_id, **kwargs):
        try:
            ...
            if not _can_upload_or_delete(user):
                ...
                return _att_error(
                    403, "forbidden", "Insufficient permissions to upload attachments."
                )
            ...
            attachment_type = request.httprequest.form.get(
                "attachment_type", ""
            ).strip()
            ...
            allowed_for_type = (
                ...
                if attachment_type == TYPE_IMAGE
                else ALLOWED_DOCUMENT_MIMETYPES
            )
            ...
            return success_response(
                {
                    ...
                    "data": _serialize_attachment(
                        att, property_id, include_self_link=True
                    ),
                },
                status_code=201,
            )
            ...
            _logger.exception(
                "upload_attachment: unexpected error for property %s", property_id
            )
    ...
    def list_attachments(self, property_id, **kwargs):
        try:
            ...
            try:
                ...
                return _att_error(
                    400, "invalid_pagination", "Invalid limit or offset value."
                )
                    ...
                    "data": {
                        "items": [
                            _serialize_attachment(
                                a, property_id, include_self_link=False
                            )
                            for a in items
                        ],
                        "pagination": {
            ...
            _logger.exception(
                "list_attachments: unexpected error for property %s", property_id
            )
            return _att_error(500, "internal_error", "An unexpected error occurred.")
    ...
    def delete_attachment(self, property_id, attachment_id, **kwargs):
        try:
            ...
            if not _can_upload_or_delete(user):
                ...
                return _att_error(
                    403, "forbidden", "Insufficient permissions to delete attachments."
                )

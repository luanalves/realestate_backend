...
class MasterDataApiController(http.Controller):
    ...
    def list_states(self, **kwargs):
        try:
            ...
            State = request.env[
                "res.country.state"
            ].sudo()  # Feature 011: native state model
    ...
    def list_companies(self, **kwargs):
        try:
            ...
            if request.user_company_ids:
                domain = [
                    ("id", "in", request.user_company_ids),
                    ("is_real_estate", "=", True),
                ]
            else:
                domain = [
                    ("is_real_estate", "=", True)
                ]  # Admin vê todas as RE companies
    ...
    @http.route(
        "/api/v1/tags", type="http", auth="none", methods=["GET"], csrf=False, cors="*"
    )
    @require_jwt
    @require_session

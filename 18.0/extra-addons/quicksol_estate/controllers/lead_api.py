...
class LeadApiController(http.Controller):
    @http.route(
        "/api/v1/leads", type="http", auth="none", methods=["GET"], csrf=False, cors="*"
    )
    @require_jwt
    @require_session
    @require_company
    def list_leads(self, **kwargs):
        ...
        try:
            ...
            if agent_filter and user.has_group(
            ...
            if property_type_filter:
                domain.append(
                    ("property_type_interest", "=", int(property_type_filter))
                )
                ...
                try:
                    ...
                    domain.append(
                        (
                            "create_date",
                            ">=",
                            dt.strptime(created_from, "%Y-%m-%d").strftime(
                                "%Y-%m-%d 00:00:00"
                            ),
                        )
                    )
                    ...
                    _logger.warning(
                        f"Invalid date format for created_from: {created_from}"
                    )
                ...
                try:
                    ...
                    domain.append(
                        (
                            "create_date",
                            "<=",
                            dt.strptime(created_to, "%Y-%m-%d").strftime(
                                "%Y-%m-%d 23:59:59"
                            ),
                        )
                    )
                ...
                try:
                    ...
                    for lead in all_leads:
                        ...
                        if not last_message or (
                            last_message.date
                            and last_message.date.date() < activity_date.date()
                        ):
            ...
            lead_ctx = (
                Lead.with_context(active_test=False) if active_filter == "all" else Lead
            )

            total = lead_ctx.sudo().search_count(domain)
            leads = lead_ctx.sudo().search(
            ...
            for lead in leads:
                lead_data = {
                    ...
                    "property_type_interest": (
                        lead.property_type_interest.name
                        if lead.property_type_interest
                        else None
                    ),
                    "first_contact_date": (
                        lead.first_contact_date.strftime("%Y-%m-%d")
                        if lead.first_contact_date
                        else None
                    ),
                    "expected_closing_date": (
                        lead.expected_closing_date.strftime("%Y-%m-%d")
                        if lead.expected_closing_date
                        else None
                    ),
                    "days_in_state": lead.days_in_state,
                    "created_at": (
                        lead.create_date.strftime("%Y-%m-%d %H:%M:%S")
                        if lead.create_date
                        else None
                    ),
                    "updated_at": (
                        lead.write_date.strftime("%Y-%m-%d %H:%M:%S")
                        if lead.write_date
                        else None
                    ),
                }
    ...
    def create_lead(self, **kwargs):
        ...
        try:
            ...
            if body_data.get("property_type_interest"):
                lead_vals["property_type_interest"] = int(
                    body_data["property_type_interest"]
                )
    ...
    def get_lead(self, lead_id, **kwargs):
        ...
        try:
            ...
            if not lead.exists():
                return error_response(
                    f"Lead with ID {lead_id} not found", 404, "NOT_FOUND"
                )
            ...
            include_activities = (
                kwargs.get("include_activities", "false").lower() == "true"
            )

            # Serialize response
            response_data = self._serialize_lead(
                lead, include_activities=include_activities
            )
    ...
    def update_lead(self, lead_id, **kwargs):
        ...
        try:
            ...
            if "agent_id" in body_data and is_agent and not is_manager:
                return error_response(
                    "Agents cannot change agent assignment", 403, "PERMISSION_DENIED"
                )
            ...
            if "agent_id" in body_data and is_manager and body_data["agent_id"]:
                ...
                if not new_agent.exists():
                    return error_response(
                        f"Agent with ID {new_agent_id} not found",
                        404,
                        "AGENT_NOT_FOUND",
                    )
                ...
                agent_company_id = (
                    new_agent.company_id.id if new_agent.company_id else None
                )
            ...
            for field in allowed_fields:
                if field in body_data:
                    if field in [
                        ...
                        update_vals[field] = (
                            int(body_data[field]) if body_data[field] else False
                        )
                    elif field in ["budget_min", "budget_max", "min_area", "max_area"]:
                        update_vals[field] = (
                            float(body_data[field]) if body_data[field] else 0.0
                        )
                    elif field == "bedrooms_needed":
                        update_vals[field] = (
                            int(body_data[field]) if body_data[field] else 0
                        )
                    else:
    ...
    @http.route(
        "/api/v1/leads/<int:lead_id>/convert",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def convert_lead(self, lead_id, **kwargs):
        ...
        try:
            ...
            if not body_data.get("property_id"):
                return error_response(
                    'Field "property_id" is required', 400, "VALIDATION_ERROR"
                )
    ...
    @http.route(
        "/api/v1/leads/<int:lead_id>/reopen",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def lead_statistics(self, **kwargs):
        ...
        try:
            ...
            if not (
                ...
                return error_response(
                    "Access denied: Manager or Owner role required",
                    403,
                    "ACCESS_DENIED",
                )
            ...
            agent_data = Lead.sudo().read_group(
                domain, fields=["agent_id"], groupby=["agent_id"]
            )
            ...
            for group in agent_data:
                ...
                by_agent.append(
                    {"agent_id": agent_id, "agent_name": agent_name, "count": count}
                )
    ...
    def _serialize_lead(self, lead, include_activities=False):
        ...
        data = {
            ...
            "property_interest": (
                lead.property_interest.id if lead.property_interest else None
            ),
            "property_interest_name": (
                lead.property_interest.name if lead.property_interest else None
            ),
            "first_contact_date": (
                lead.first_contact_date.strftime("%Y-%m-%d")
                if lead.first_contact_date
            ...
            "lost_date": (
                lead.lost_date.strftime("%Y-%m-%d") if lead.lost_date else None
            ),
            "lost_reason": lead.lost_reason,
            "converted_property_id": (
                lead.converted_property_id.id if lead.converted_property_id else None
            ),
            "converted_sale_id": (
                lead.converted_sale_id.id if lead.converted_sale_id else None
            ),
            "days_in_state": lead.days_in_state,
            "created_at": (
                lead.create_date.strftime("%Y-%m-%d %H:%M:%S")
        ...
        if include_activities:
            ...
            for msg in messages:
                ...
                clean_body = re.sub(
                    r"[📞📧🤝📝]\s*(CALL|EMAIL|MEETING|NOTE)\s*", "", clean_body
                ).strip()
                ...
                recent_activities.append(
                    {
                        ...
                        "date": (
                            msg.date.strftime("%Y-%m-%d %H:%M:%S") if msg.date else None
                        ),
                    }
                )
    ...
    def log_activity(self, lead_id, **kwargs):
        ...
        try:
            ...
            is_agent = (
                "Estate Agent" in user_groups and "Estate Manager" not in user_groups
            )

            if is_agent and lead.agent_id.id != current_user.id:
                return error_response(
                    "Forbidden",
                    "Access denied: You can only log activities on your own leads",
                    403,
                )
            ...
            formatted_body = (
                f"{icon} <strong>{activity_type.upper()}</strong><br/>{body}"
            )
            ...
            activity_data = {
                ...
                "created_at": (
                    message.create_date.strftime("%Y-%m-%d %H:%M:%S")
            ...
            _logger.error(
                f"Error logging activity for lead {lead_id}: {str(e)}", exc_info=True
            )
    ...
    def list_activities(self, lead_id, **kwargs):
        ...
        try:
            ...
            if is_agent and lead.agent_id.id != current_user.id:
                return error_response(
                    403,
                    "Access denied: You can only view activities on your own leads",
                    "FORBIDDEN",
                )
                ...
                activities.append(
                    {
                        ...
                        "created_at": (
                            msg.create_date.strftime("%Y-%m-%d %H:%M:%S")
                            if msg.create_date
                            else None
                        ),
                    }
            ...
            _logger.error(
                f"Error listing activities for lead {lead_id}: {str(e)}", exc_info=True
            )
    ...
    def schedule_activity(self, lead_id, **kwargs):
        ...
        try:
            ...
            try:
                ...
                return error_response(
                    "Validation Error", "Invalid date format. Use YYYY-MM-DD", 400
                )
            ...
            if is_agent and lead.agent_id.id != current_user.id:
                return error_response(
                    "Forbidden",
                    "Access denied: You can only schedule activities on your own leads",
                    403,
                )
            ...
            if not assigned_user.exists():
                return error_response(
                    "Validation Error", f"User with ID {user_id} not found", 400
                )
            ...
            activity_vals = {
                ...
                .sudo()
                .search([("model", "=", "real.estate.lead")], limit=1)
                .id,
            ...
            activity_data = {
                ...
                "created_at": (
                    activity.create_date.strftime("%Y-%m-%d %H:%M:%S")
            ...
            _logger.error(
                f"Error scheduling activity for lead {lead_id}: {str(e)}", exc_info=True
            )
            return error_response("Server Error", str(e), 500)
    ...
    def create_filter(self, **kwargs):
        ...
        try:
            ...
            if not name:
                return error_response(
                    "Validation Error", "Filter name is required", 400
                )
            ...
            filter_data = {
                ...
                "created_at": (
                    filter_record.create_date.strftime("%Y-%m-%d %H:%M:%S")
                    if filter_record.create_date
                    else None
                ),
            }

            return success_response("Filter saved successfully", filter_data, 201)
    ...
    def list_filters(self, **kwargs):
        ...
        try:
            ...
            response_data = {
                ...
                "data": {"filters": filter_list, "total": len(filter_list)},
            }
    ...
    @http.route(
        "/api/v1/leads/filters/<int:filter_id>",
        type="json",
        auth="none",
        methods=["DELETE"],
        csrf=False,
    ...
    def delete_filter(self, filter_id, **kwargs):
        ...
        try:
            ...
            if filter_record.user_id.id != request.env.user.id:
                return error_response(
                    "Forbidden", "You can only delete your own filters", 403
                )
            ...
            return success_response(
                f'Filter "{filter_name}" deleted successfully', {"id": filter_id}, 200
            )

...
from ..utils.responses import success_response as util_success, build_hateoas_links
...
class OwnerApiController(http.Controller):
        ...
        try:
            ...
            try:
                ...
                if not cpf_validator.validate(cpf_clean):
                    return error_response(
                        400,
                        f"Invalid CPF: {data['cpf']}. CPF must have 11 valid digits.",
                    )
            except ImportError:
                ...
                if len(cpf_clean) != 11:
                    return error_response(
                        400, f"Invalid CPF: {data['cpf']}. CPF must have 11 digits."
                    )
    ...
    @http.route(
        "/api/v1/owners/<int:owner_id>/companies",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
        ...
        try:
            ...
            if not user.has_group("base.group_system"):
                if company_id not in user.company_ids.ids:
                    return error_response(
                        403, "You can only link Owners to your own companies"
                    )

            # Check if already linked
            if company_id in owner.company_ids.ids:
                return error_response(
                    409, f"Owner already linked to company '{company.name}'"
                )

            # Link owner to company
            owner.write({"company_ids": [(4, company_id)]})  # Add to Many2many
            ...
            links = build_hateoas_links(
                "/api/v1/owners", owner_id, {"companies": "/companies"}
            )
            ...
            _logger.error(
                f"Error linking owner {owner_id} to company: {str(e)}", exc_info=True
            )
            return error_response(500, "Internal server error")
    ...
    @http.route(
        "/api/v1/owners/<int:owner_id>/companies/<int:company_id>",
        type="http",
        auth="none",
        methods=["DELETE"],
        csrf=False,
    ...
    def unlink_owner_from_company(self, owner_id, company_id, **kwargs):
        ...
        try:
            ...
            if not user.has_group("base.group_system"):
                if company_id not in user.company_ids.ids:
                    return error_response(
                        403, "You can only unlink Owners from your own companies"
                    )

            # Check if owner is linked to company
            if company_id not in owner.company_ids.ids:
                return error_response(
                    404, f"Owner not linked to company '{company.name}'"
                )
            ...
            owner.write({"company_ids": [(3, company_id)]})  # Remove from Many2many
            ...
            _logger.error(
                f"Error unlinking owner {owner_id} from company {company_id}: {str(e)}",
                exc_info=True,
            )
            return error_response(500, "Internal server error")

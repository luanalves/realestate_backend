...
def _load_schema(name):
    ...
    schema_path = os.path.join(
        os.path.dirname(__file__), "..", "schemas", f"{name}.json"
    )
...
def _serialize_proposal(proposal, include_chain=False, include_attachments=False):
    ...
    data = {
        ...
        "accepted_date": (
            proposal.accepted_date.isoformat() if proposal.accepted_date else None
        ),
        "rejected_date": (
            proposal.rejected_date.isoformat() if proposal.rejected_date else None
        ),
        "rejection_reason": proposal.rejection_reason or None,
        "cancellation_reason": proposal.cancellation_reason or None,
        "parent_proposal_id": (
            proposal.parent_proposal_id.id if proposal.parent_proposal_id else None
        ),
        "superseded_by_id": (
            proposal.superseded_by_id.id if proposal.superseded_by_id else None
        ),
        "queue_position": (
            proposal.queue_position if proposal.queue_position >= 0 else None
        ),
        ...
        "created_at": (
            proposal.create_date.isoformat() if proposal.create_date else None
        ),
...
def _build_hateoas_links(proposal):
    ...
    is_manager_or_owner = user.has_group(
        "quicksol_estate.group_real_estate_manager"
    ) or user.has_group("quicksol_estate.group_real_estate_owner")

    if proposal.state == "draft":
        links.append(
            {"rel": "send", "href": f"{base}/{proposal.id}/send", "method": "POST"}
        )
        links.append(
            {"rel": "cancel", "href": f"{base}/{proposal.id}", "method": "DELETE"}
        )
    elif proposal.state == "sent":
        links.append(
            {
                "rel": "counter",
                "href": f"{base}/{proposal.id}/counter",
                "method": "POST",
            }
        )
        if is_manager_or_owner:
            links.append(
                {
                    "rel": "accept",
                    "href": f"{base}/{proposal.id}/accept",
                    "method": "POST",
                }
            )
            links.append(
                {
                    "rel": "reject",
                    "href": f"{base}/{proposal.id}/reject",
                    "method": "POST",
                }
            )
        if is_manager_or_owner:
        ...
        links.append(
            {"rel": "renew", "href": f"{base}/{proposal.id}/renew", "method": "POST"}
        )

    links.append({"rel": "self", "href": f"{base}/{proposal.id}", "method": "GET"})
    links.append(
        {"rel": "queue", "href": f"{base}/{proposal.id}/queue", "method": "GET"}
    )
    return links
...
class ProposalController(http.Controller):
        ...
        try:
            ...
            if errors:
                return error_response(
                    "VALIDATION_ERROR", "Validation failed", 400, details=errors
                )
            ...
            return success_response(
                _serialize_proposal(
                    proposal, include_chain=False, include_attachments=False
                ),
                status_code=201,
            )
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    def get_proposal(self, proposal_id, **kwargs):
        try:
            ...
            return success_response(
                _serialize_proposal(
                    proposal, include_chain=True, include_attachments=True
                )
            )
        except AccessError:
            return error_response("NOT_FOUND", _("Proposal not found."), 404)
    ...
    def update_proposal(self, proposal_id, **kwargs):
        try:
            ...
            if proposal.state in ("accepted", "rejected", "expired", "cancelled"):
                return error_response(
                    "CONFLICT", _("Cannot update a proposal in terminal state."), 409
                )
            ...
            if "agent_id" in body:
                ...
                if not (
                    ...
                    return error_response(
                        "FORBIDDEN", _("Only managers/owners can reassign agents."), 403
                    )
    ...
    def cancel_proposal(self, proposal_id, **kwargs):
        try:
            ...
            if not (
                ...
                return error_response(
                    "FORBIDDEN", _("Only managers/owners can cancel proposals."), 403
                )
            ...
            if not reason:
                return error_response(
                    "VALIDATION_ERROR", _("cancellation_reason is required."), 400
                )
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/send",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/accept",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def accept_proposal(self, proposal_id, **kwargs):
        try:
            ...
            return error_response(
                "FORBIDDEN" if isinstance(e, AccessError) else "UNPROCESSABLE",
                str(e),
                status,
            )
        except Exception:
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/reject",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/counter",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/queue",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
            ...
            return success_response(
                {
                    "property_id": property_id,
                    "active_proposal": (
                        _serialize_proposal(active_proposal)
                        if active_proposal
                        else None
                    ),
    ...
    @http.route(
        "/api/v1/proposals/<int:proposal_id>/attachments",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    def upload_attachment(self, proposal_id, **kwargs):
        try:
            ...
            if len(content) > MAX_ATTACHMENT_BYTES:
                return error_response(
                    "PAYLOAD_TOO_LARGE", _("File exceeds the 10 MB limit."), 413
                )
            ...
            if mimetype not in ALLOWED_ATTACHMENT_MIMETYPES:
                return error_response(
                    "VALIDATION_ERROR",
                    _('File type "%s" is not allowed.') % mimetype,
                    400,
                )
            ...
            _logger.exception(
                "POST /api/v1/proposals/%d/attachments failed", proposal_id
            )
            return error_response("INTERNAL_ERROR", _("Internal server error."), 500)

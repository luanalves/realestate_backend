...
def _serialize_source(source):
    return {
        ...
        "links": [
            {
                "rel": "self",
                "href": f"/api/v1/service-sources/{source.id}",
                "method": "GET",
            },
            {
                "rel": "update",
...
class ServiceSourceController(http.Controller):
        ...
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can create sources.", 403
                )
            body = json.loads(request.httprequest.data or "{}")
            if not body.get("name") or not body.get("code"):
                return error_response(
                    "VALIDATION_ERROR", "name and code are required.", 400
                )
            vals = {
                "name": body["name"],
    ...
    @http.route(
        "/api/v1/service-sources/<int:source_id>",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    def update_source(self, source_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can update sources.", 403
                )
            source = request.env["real.estate.service.source"].browse(source_id)
            if not source.exists():
                return error_response("NOT_FOUND", "Source not found.", 404)
    ...
    def delete_source(self, source_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can delete sources.", 403
                )
            source = request.env["real.estate.service.source"].browse(source_id)
            if not source.exists():
                return error_response("NOT_FOUND", "Source not found.", 404)

...
def _serialize_tag(tag):
    return {
        ...
        "links": [
            ...
            {
                "rel": "update",
                "href": f"/api/v1/service-tags/{tag.id}",
                "method": "PUT",
            },
            {
                "rel": "delete",
...
class ServiceTagController(http.Controller):
        ...
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can create tags.", 403
                )
    ...
    @http.route(
        "/api/v1/service-tags/<int:tag_id>",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    def update_tag(self, tag_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can update tags.", 403
                )
            tag = request.env["real.estate.service.tag"].browse(tag_id)
            if not tag.exists():
                return error_response("NOT_FOUND", "Tag not found.", 404)
    ...
    def delete_tag(self, tag_id, **kwargs):
        try:
            if not _is_manager_or_owner(request.env):
                return error_response(
                    "FORBIDDEN", "Only Owner/Manager can delete tags.", 403
                )
            tag = request.env["real.estate.service.tag"].browse(tag_id)
            if not tag.exists():
                return error_response("NOT_FOUND", "Tag not found.", 404)
            if tag.is_system:
                return error_response(
                    "FORBIDDEN", "System tags cannot be deleted.", 403
                )

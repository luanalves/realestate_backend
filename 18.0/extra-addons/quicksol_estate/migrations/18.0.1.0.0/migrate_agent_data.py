...
class AgentDataMigration:
    ...
    def migrate_pre(self):
        ...
        try:
            ...
            if agents_without_company:
                _logger.warning(
                    f"Found {len(agents_without_company)} agents without company assignment"
                )
    ...
    def _assign_default_companies(self):
        ...
        _logger.info(
            f"Assigning {len(agents_without_company)} agents to company: {default_company.name}"
        )
        ...
        _logger.info(
            f"Successfully assigned {len(agents_without_company)} agents to default company"
        )
    ...
    def _set_default_active_status(self):
        ...
        if null_count > 0:
            _logger.info(
                f"Setting active=True for {null_count} agents with NULL status"
            )
            ...
            _logger.info(
                f"Updated {null_count} agents to active=True and invalidated cache"
            )
        else:
...
def main():
    ...
    print("Agent Data Migration Script")

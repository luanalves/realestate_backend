...
class AgentService:
    ...
    def create_agent(self, values, company_id=None):
        ...
        if existing_agent:
            raise ValidationError(
                _("An agent with CRECI %s already exists in this company")
                % values["creci"]
            )
        ...
        try:
            ...
            _logger.info(
                f"Created agent {agent.name} (ID: {agent.id}) for company {company_id}"
            )
            return agent
    ...
    def deactivate_agent(self, agent_id, reason=None, user_company_id=None):
        ...
        try:
            ...
            _logger.info(
                f"Deactivated agent {agent.name} (ID: {agent.id}) - Reason: {reason}"
            )
            return agent

...
class CreciValidator:
    ...
    def normalize(cls, creci_input):
        ...
        if not state or not number:
            raise ValidationError(
                _(
                    "Formato de CRECI inválido: %s. Formatos aceitos: CRECI/UF 12345, CRECI-UF-12345, 12345-UF"
                )
                % creci_input
            )
        ...
        if not number.isdigit() or len(number) < 4 or len(number) > 8:
            raise ValidationError(
                _("Número CRECI inválido: %s. Deve conter entre 4 e 8 dígitos") % number
            )
    ...
    def validate(cls, creci_normalized):
        ...
        if state not in cls.VALID_STATES:
            raise ValidationError(_("Estado inválido no CRECI: %s") % state)

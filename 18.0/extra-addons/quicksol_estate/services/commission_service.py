...
class CommissionService:
    ...
    def calculate_commission(self, rule, transaction_amount):
        ...
        if transaction_amount < rule.min_value:
            raise ValidationError(
                _(
                    "Transaction amount (%.2f) is below minimum value (%.2f) for this rule"
                )
        ...
        if transaction_amount > rule.max_value:
            raise ValidationError(
                _(
                    "Transaction amount (%.2f) exceeds maximum value (%.2f) for this rule"
                )
    ...
    def get_active_rule_for_agent(
        self, agent_id, transaction_type, transaction_date=None
    ):
        ...
        domain = [
            ...
            (
                ...
                "<=",
                transaction_date,
            ),  # Non-retroactive: must be valid from this date
        ]
    ...
    def create_commission_transaction(
        ...
        # Get active rule
        rule = self.get_active_rule_for_agent(
        ...
        if not rule:
            ...
            raise UserError(
                _(
                    "No active commission rule found for agent %(agent)s and transaction type %(type)s "
    ...
    def get_agent_commission_summary(self, agent_id, date_from=None, date_to=None):
        ...
        if (
            ...
            and agent.company_id not in user_accessible_companies
        ):
            raise AccessError(
        ...
        return {
            ...
            "cancelled_commission": total_commission
            - paid_commission
            - pending_commission,

...
class AssignmentService:
    ...
    def assign_agent_to_property(
        ...
        if responsibility_type not in valid_types:
            raise ValidationError(
                _("Invalid responsibility type: %s. Must be one of: %s")
        ...
        if company_id:
            ...
            if agent.company_id != company:
                raise AccessError(
                    _("Cannot assign agent from company %s using company %s context")
        ...
        if existing_assignment:
            ...
            return (
                existing_assignment,
                False,
            )  # Returns tuple: (assignment, created=False)
    ...
    def get_agent_properties(self, agent_id, active_only=True, company_id=None):
        ...
        if company_id:
            ...
            if agent.company_id != company:
                raise AccessError(
                    _("Cannot access agent from company %s using company %s context")
                    % (agent.company_id.name, company.name)
                )
    ...
    def get_property_agents(self, property_id, active_only=True, company_id=None):
        ...
        if company_id:
            ...
            if company != property_obj.company_id:
                raise AccessError(
                    _("Cannot access property from company %s using company %s context")
                    % (
                        (
                            property_obj.company_id.name
    ...
    def bulk_assign_agent(
        self, agent_id, property_ids, responsibility_type="primary", company_id=None
    ):
        ...
        for property_id in property_ids:
            try:
                ...
                results["errors"].append({"property_id": property_id, "error": str(e)})
                _logger.warning(

...
class TestActivityTracking(common.TransactionCase):
    ...
    def test_activity_logging_via_message_post(self):
        ...
        # Log an activity
        activity_body = (
            "📞 <strong>CALL</strong><br/>Called client to discuss property options"
        )

        message = self.lead.with_user(self.agent).message_post(
            body=activity_body, message_type="comment", subtype_xmlid="mail.mt_note"
        )

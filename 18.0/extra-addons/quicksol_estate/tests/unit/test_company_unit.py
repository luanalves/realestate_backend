...
class TestCompanyUnit(BaseCompanyTest):
    ...
    def test_company_computed_property_count(self):
        ...
        company = self.create_mock_record(
            "res.company", {"name": "Test Company", "property_ids": mock_properties}
        )

        # Act - Simulate computed field logic
        property_count = (
            len(company.property_ids) if hasattr(company, "property_ids") else 0
        )
        ...
        self.assertEqual(
            property_count,
            3,
            "Property count should match number of related properties",
        )
    ...
    def test_company_computed_agent_count(self):
        ...
        company = self.create_mock_record(
            "res.company", {"name": "Test Company", "agent_ids": mock_agents}
        )
        ...
        # Assert
        self.assertEqual(
            agent_count, 2, "Agent count should match number of related agents"
        )
    ...
    def test_company_computed_lease_count(self):
        ...
        # Assert
        self.assertEqual(
            lease_count, 1, "Lease count should match number of related leases"
        )
    ...
    def test_company_computed_sale_count(self):
        ...
        # Assert
        self.assertEqual(
            sale_count, 1, "Sale count should match number of related sales"
        )
    ...
    def test_company_cnpj_validation_invalid(self):
        ...
        for invalid_cnpj in invalid_cnpjs:
            with self.subTest(cnpj=invalid_cnpj):
                # Arrange
                company = self.create_mock_record(
                    "res.company", {"name": "Test Company", "cnpj": invalid_cnpj}
                )
                ...
                if invalid_cnpj and invalid_cnpj != "":  # Empty is allowed
                    self.assertTrue(
                        should_raise_error,
                        f"Invalid CNPJ {invalid_cnpj} should trigger validation error",
                    )
    ...
    def test_company_string_representation(self):
        ...
        # Arrange
        company = self.create_mock_record(
            "res.company", {"name": "Premium Real Estate Solutions"}
        )
        ...
        self.assertTrue(
            len(display_name) > 0, "Company display name should not be empty"
        )
    ...
    def test_company_many2many_relationships(self):
        ...
        # Arrange
        company = self.create_mock_record(
            "res.company", {"name": "Relationship Test Company"}
        )
        ...
        # Act & Assert
        self.assertEqual(
            len(company.property_ids), 3, "Should have 3 related properties"
        )
...
class TestCompanyBusinessLogic(BaseCompanyTest):
    ...
    def test_company_active_filtering(self):
        ...
        # Arrange
        active_company = self.create_mock_record(
            "res.company", {"name": "Active Company", "active": True}
        )

        inactive_company = self.create_mock_record(
            "res.company", {"name": "Inactive Company", "active": False}
        )
    ...
    def test_company_data_integrity(self):
        ...
        self.assertTrue(
            company.name and len(company.name.strip()) > 0,

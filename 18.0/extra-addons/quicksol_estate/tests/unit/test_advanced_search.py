...
class TestAdvancedSearch(common.TransactionCase):
    ...
    def setUp(self):
        ...
        # Create test company
        self.company = self.Company.create({"name": "Test Real Estate Company"})
        ...
        self.apt_type = self.PropertyType.create({"name": "Apartment"})
    ...
    def test_budget_range_filter(self):
        ...
        domain = [("budget_max", ">=", 250000), ("budget_min", "<=", 450000)]
        results = self.Lead.search(domain)
    ...
    def test_sorting_by_budget(self):
        ...
        indices = [
            results_asc.ids.index(lead.id)
            for lead in [self.lead_low, self.lead_mid, self.lead_high]
        ]
        ...
        indices_desc = [
            results_desc.ids.index(lead.id)
            for lead in [self.lead_high, self.lead_mid, self.lead_low]
        ]
    ...
    def test_empty_results(self):
        ...
        domain = [("budget_min", ">", 1000000), ("budget_max", "<", 100000)]
        results = self.Lead.search(domain)

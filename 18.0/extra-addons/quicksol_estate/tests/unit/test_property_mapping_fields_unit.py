...
SPEC = importlib.util.spec_from_file_location(
    "property_mapping_serializers", SERIALIZERS_PATH
)
...
OPTIONS_SPEC = importlib.util.spec_from_file_location(
    "property_mapping_options", PROPERTY_OPTIONS_PATH
)
...
class TestPropertyMappingValues(unittest.TestCase):
    def test_build_values_accepts_valid_mapping_fields(self):
        ...
        self.assertEqual(
            vals["fgts_usage_notes"], "Uso identificado na matricula anterior"
        )
...
class TestSerializePropertyMappingFields(unittest.TestCase):
    ...
    def test_serialize_mapping_fields_returns_fgts_object_without_duplicate_scalars(
        self,
    ):
        property_record = _property_record(
            accepts_fgts=True,
...
class TestReplacePropertyAttachments(unittest.TestCase):
    def test_replace_property_files_rejects_missing_file_before_unlink(self):
        ...
        errors = serializers._replace_property_files(
            property_record,
            [
                {
        ...
        self.assertEqual(
            property_record.env["real.estate.property.document"].created, []
        )

    def test_replace_property_images_rejects_invalid_item_before_unlink(self):
        property_record = _property_record_with_relations()

        errors = serializers._replace_property_images(
            property_record, ["not-an-object"]
        )

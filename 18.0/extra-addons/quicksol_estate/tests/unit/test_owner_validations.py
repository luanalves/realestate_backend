...
class TestCreatorValidation(TransactionCase):
    ...
    def test_01_owner_can_create_owner(self):
        ...
        self.assertTrue(
            new_owner.exists(), "Owner should be able to create new Owner users"
        )
        self.assertIn(
            self.group_owner, new_owner.groups_id, "New user should have Owner group"
        )
    ...
    def test_02_admin_can_create_owner(self):
        ...
        self.assertTrue(
            new_owner.exists(), "Admin should be able to create new Owner users"
        )
        self.assertIn(
            self.group_owner, new_owner.groups_id, "New user should have Owner group"
        )
    ...
    def test_03_manager_cannot_create_owner(self):
        ...
        with self.assertRaises(
            AccessError, msg="Manager should not be able to create Owner users"
        ):
...
class TestLastOwnerProtection(TransactionCase):
    ...
    def test_01_cannot_delete_last_owner_of_any_company(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should not delete last owner of Company A"
        ):
    ...
    def test_02_can_delete_non_last_owner(self):
        ...
        self.assertFalse(
            self.owner2.active, "Owner2 should be soft-deleted successfully"
        )
        ...
        self.assertTrue(
            len(active_owners) >= 1, "Company B should still have active owners"
        )
    ...
    def test_03_cannot_unlink_last_owner_from_company(self):
        ...
        # Owner1 is the ONLY owner of Company A
        with self.assertRaises(
            ValidationError, msg="Should not unlink last owner from Company A"
        ):
            self.owner1.write(
    ...
    def test_04_can_unlink_non_last_owner_from_company(self):
        ...
        self.assertIn(
            self.company_b,
            original_companies,
            "Owner2 should initially be in Company B",
        )
    ...
    def test_05_last_owner_check_ignores_inactive_owners(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should not delete last ACTIVE owner"
        ):
            self.owner1.write({"active": False})

...
class TestCNPJValidation(TransactionCase):
    ...
    def test_03_invalid_cnpj_wrong_check_digit(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject CNPJ with wrong check digits"
        ):
    ...
    def test_04_invalid_cnpj_wrong_length(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject CNPJ with wrong length"
        ):
            self.env["res.company"].create(
                {
                    "name": "Wrong Length",
    ...
    def test_06_cnpj_uniqueness_includes_inactive(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject CNPJ that exists in inactive records"
        ):
            self.env["res.company"].create(
                {
                    "name": "New Company",
    ...
    def test_07_cnpj_all_zeros_invalid(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject CNPJ with all zeros"
        ):
            self.env["res.company"].create(
                {
                    "name": "All Zeros",
...
class TestEmailValidation(TransactionCase):
    ...
    def test_05_invalid_email_no_domain(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject email without domain"
        ):
            self.env["res.company"].create(
                {
                    "name": "No Domain",
    ...
    def test_07_invalid_email_double_at(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject email with double @"
        ):
            self.env["res.company"].create(
                {
                    "name": "Double At",

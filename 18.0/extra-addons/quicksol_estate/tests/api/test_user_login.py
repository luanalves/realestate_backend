    ...
    try:
        ...
        if response.status_code != 200:
            return print_test_result(
                False, f"Esperado 200, recebido {response.status_code}"
            )
...
def test_03_login_invalid_credentials(base_url):
    ...
    try:
        ...
        if response.status_code != 200:
            return print_test_result(
                False,
                f"Esperado 200 (Odoo retorna 200 mesmo com erro), recebido {response.status_code}",
            )
...
def test_04_logout_user(base_url, user_email, user_password):
    ...
    try:
        ...
        if logout_response.status_code != 200:
            return print_test_result(
                False, f"Esperado 200, recebido {logout_response.status_code}"
            )
...
def test_05_change_profile_user_a(base_url, user_a_email, user_a_password):
    ...
    try:
        ...
        if update_response.status_code != 200:
            return print_test_result(
                False, f"Esperado 200, recebido {update_response.status_code}"
            )
        ...
        if "error" in update_data:
            ...
            if "not allowed to modify" in error_msg or "Access Rights" in error_msg:
                print("  Acesso negado (esperado - requer permissão Admin)")
                return print_test_result(
                    True, "✓ Odoo protege alteração de usuários (requer Admin)"
                )
            else:
        ...
        print(f"  UID: {uid}")
        print("  Telefone atualizado: +55 11 98888-7777")

        return print_test_result(
            True, "User A atualizou seu próprio perfil (tem permissão Admin)"
        )
...
def test_06_user_a_tries_change_user_b_profile(
    base_url, user_a_email, user_a_password, user_b_email, user_b_password
):
    ...
    try:
        ...
        print("  User A tentando alterar telefone do User B...")
        ...
        if attack_response.status_code != 200:
            return print_test_result(
                True,
                f"Ataque bloqueado no HTTP level (status {attack_response.status_code})",
            )
        ...
        if "error" in attack_data:
            error_msg = (
                attack_data["error"].get("data", {}).get("message", "Unknown error")
            )
            print(f"  Erro retornado: {error_msg}")
            return print_test_result(
                True, "✓ SEGURANÇA OK: User A não conseguiu alterar perfil do User B"
            )

        # Se chegou aqui e deu sucesso, é uma falha de segurança!
        if attack_data.get("result") is True:
            return print_test_result(
                False,
                "✗ FALHA DE SEGURANÇA: User A conseguiu alterar perfil do User B!",
            )
...
def test_07_user_a_tries_read_user_b_session(
    base_url, user_a_email, user_a_password, user_b_email, user_b_password
):
    ...
    try:
        ...
        print("  User A tentando usar session_id do User B...")

        hijack_response = session_hijack.post(
            f"{base_url}/web/session/get_session_info",
            json={"jsonrpc": "2.0", "method": "call", "params": {}, "id": 4},
            headers={
        ...
        if hijack_response.status_code != 200:
            return print_test_result(
                True, f"Tentativa bloqueada (HTTP {hijack_response.status_code})"
            )
        ...
        if result.get("username") == user_b_email:
            return print_test_result(
                False, "✗ FALHA DE SEGURANÇA: User A conseguiu usar sessão do User B!"
            )

        # Se retornou info do User A ou erro, está correto
        if result.get("username") == user_a_email:
            return print_test_result(
                True, "✓ SEGURANÇA OK: Sessão não foi sequestrada (ainda logado como A)"
            )
...
def test_08_get_user_me(
    base_url, user_a_email, user_a_password, oauth_client_id, oauth_client_secret
):
    ...
    try:
        # Obtém OAuth token
        oauth_token, success = get_oauth_token(
        ...
        login_response = requests.post(
            ...
            json={
                "jsonrpc": "2.0",
                "method": "call",
                "params": {"email": user_a_email, "password": user_a_password},
                "id": 1,
            },
            headers={
                "Content-Type": "application/json",
        ...
        print(
            f"  API session_id: {api_session_id[:20] if api_session_id else 'N/A'}..."
        )
        print(
            f"  Cookie session_id: {cookie_session_id[:20] if cookie_session_id else 'N/A'}..."
        )
        ...
        if me_response.status_code != 200:
            return print_test_result(
                False, f"Esperado 200, recebido {me_response.status_code}"
            )
        ...
        if missing_fields:
            return print_test_result(
                False, f"Campos faltando: {', '.join(missing_fields)}"
            )
        ...
        if user.get("email") != user_a_email:
            return print_test_result(
                False,
                f"Email incorreto: esperado {user_a_email}, recebido {user.get('email')}",
            )
...
def test_09_get_user_me_without_session(base_url, oauth_client_id, oauth_client_secret):
    ...
    try:
        ...
        if me_response.status_code != 200:
            return print_test_result(
                True, f"Requisição bloqueada (HTTP {me_response.status_code})"
            )
        ...
        if "error" in data:
            ...
            if isinstance(error, dict):
                error_msg = error.get("data", {}).get("message", "") or error.get(
                    "message", ""
                )
            else:
                error_msg = str(error)
            ...
            if (
                ...
                return print_test_result(
                    True, "✓ SEGURANÇA OK: Requisição sem sessão foi rejeitada"
                )
        ...
        if result.get("user", {}).get("id"):
            return print_test_result(
                False, "✗ FALHA DE SEGURANÇA: Endpoint acessível sem sessão!"
            )
...
def test_10_api_session_hijacking(
    base_url,
    user_a_email,
    user_a_password,
    user_b_email,
    ...
    print_test_header(
        10, "API Session Hijacking - User A tenta session do User B (SECURITY TEST)"
    )
    ...
    try:
        ...
        login_b_response = requests.post(
            ...
            json={
                "jsonrpc": "2.0",
                "method": "call",
                "params": {"email": user_b_email, "password": user_b_password},
                "id": 1,
            },
            headers={
                "Content-Type": "application/json",
        ...
        print("  User A tentando usar session do User B em /api/v1/me...")
        ...
        if me_response.status_code != 200:
            return print_test_result(
                True,
                f"✓ SEGURANÇA OK: Requisição bloqueada (HTTP {me_response.status_code})",
            )
        ...
        if returned_user_id == user_b_id:
            print(
                f"  Retornou User B (ID: {returned_user_id}, Email: {returned_email})"
            )
            return print_test_result(
                False,
                "✗ FALHA DE SEGURANÇA: Session hijacking permitido! User A conseguiu acessar dados como User B",
            )
        ...
        if "error" in result or "error" in data:
            ...
            print(f"  Erro retornado: {error_msg}")
            return print_test_result(
                True, "✓ SEGURANÇA OK: Tentativa de hijacking bloqueada"
            )
...
def main():
    ...
    if (
        not user_a_email
        or not user_a_password
        or not user_b_email
        or not user_b_password
    ):
        print("\n✗ ERRO: Credenciais de usuários de teste não configuradas no .env")
        print(
            "  Necessário: TEST_USER_A_EMAIL, TEST_USER_A_PASSWORD, TEST_USER_B_EMAIL, TEST_USER_B_PASSWORD"
        )
        sys.exit(1)
    ...
    results.append(
        test_05_change_profile_user_a(base_url, user_a_email, user_a_password)
    )
    results.append(
        test_06_user_a_tries_change_user_b_profile(
            base_url, user_a_email, user_a_password, user_b_email, user_b_password
        )
    )
    results.append(
        test_07_user_a_tries_read_user_b_session(
            base_url, user_a_email, user_a_password, user_b_email, user_b_password
        )
    )
    results.append(
    ...
    results.append(
        test_09_get_user_me_without_session(
            base_url, oauth_client_id, oauth_client_secret
        )
    )
    results.append(
        test_10_api_session_hijacking(
            base_url,
            user_a_email,
            user_a_password,
            user_b_email,

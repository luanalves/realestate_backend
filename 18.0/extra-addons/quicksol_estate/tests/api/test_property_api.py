...
class TestPropertyAPI(HttpCase):
    ...
    def setUpClass(cls):
        ...
        from odoo.addons.thedevkitchen_apigateway.models.oauth_application import (
            _PLAINTEXT_CACHE,
        )
        ...
        if cached_data:
            ...
            raise Exception(
                f"Client secret not found in cache for application {cls.oauth_app.id}"
            )
    ...
    def test_01_serialize_property(self):
        ...
        from odoo.addons.quicksol_estate.controllers.property_api import (
            _serialize_property,
        )
    ...
    def test_02_validate_access_admin(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.admin_user, "read"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_company2, self.admin_user, "delete"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)
    ...
    def test_03_validate_access_manager_own_company(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.manager_user, "read"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_other, self.manager_user, "write"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_agent, self.manager_user, "delete"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)
    ...
    def test_04_validate_access_manager_other_company(self):
        ...
        has_access, error = _validate_property_access(
            self.property_company2, self.manager_user, "read"
        )
    ...
    def test_04b_validate_access_owner_own_company(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.owner_user, "read"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_other, self.owner_user, "write"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_agent, self.owner_user, "delete"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)
    ...
    def test_04c_validate_access_owner_other_company(self):
        ...
        has_access, error = _validate_property_access(
            self.property_company2, self.owner_user, "read"
        )
        self.assertFalse(has_access)
        self.assertEqual(error, "Property does not belong to your companies")
    ...
    def test_05_validate_access_agent_own_property(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.agent_user, "read"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_agent, self.agent_user, "write"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)
    ...
    def test_06_validate_access_agent_other_property(self):
        ...
        has_access, error = _validate_property_access(
            self.property_other, self.agent_user, "read"
        )
        self.assertFalse(has_access)
    ...
    def test_07_validate_access_agent_cannot_delete(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.agent_user, "delete"
        )
        self.assertFalse(has_access)
    ...
    def test_08_validate_access_user_own_company(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.user_normal, "read"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)

        has_access, error = _validate_property_access(
            self.property_other, self.user_normal, "write"
        )
        self.assertTrue(has_access)
        self.assertIsNone(error)
    ...
    def test_09_validate_access_user_cannot_delete(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.user_normal, "delete"
        )
        self.assertFalse(has_access)
    ...
    def test_10_validate_access_portal_denied(self):
        ...
        has_access, error = _validate_property_access(
            self.property_agent, self.portal_user, "read"
        )
        self.assertFalse(has_access)
...
class TestPropertyAPIHTTP(HttpCase):
    ...
    def test_get_property_not_found(self):
        ...
        response = self.url_open(
            "/api/v1/properties/999999", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_create_property_missing_required_fields(self):
        ...
        # Missing 'name' field
        payload = {"price": 300000.00, "property_type_id": self.property_type.id}

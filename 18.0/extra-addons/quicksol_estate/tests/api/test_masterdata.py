...
def test_02_property_types_without_token(base_url):
    ...
    try:
        ...
        if response.status_code != 401:
            return print_test_result(
                False, f"Esperado 401, recebido {response.status_code}"
            )
...
def test_04_states_with_oauth(base_url, token):
    ...
    try:
        response = requests.get(
            f"{base_url}/api/v1/states", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        if len(data) > 0:
            first_state = data[0]
            print(
                f"  Primeiro estado: {first_state.get('name')} ({first_state.get('code')})"
            )
...
def test_05_agents_with_oauth(base_url, token):
    ...
    try:
        response = requests.get(
            f"{base_url}/api/v1/agents", headers={"Authorization": f"Bearer {token}"}
        )
...
def test_06_company_isolation_user_a(base_url, user_a_email, user_a_password):
    ...
    try:
        ...
        session_data, success = get_user_session(
            base_url, user_a_email, user_a_password
        )
        ...
        return print_test_result(
            True, "User A consegue acessar master data da sua company"
        )
...
def main():
    ...
    if user_a_email and user_a_password:
        results.append(
            test_06_company_isolation_user_a(base_url, user_a_email, user_a_password)
        )

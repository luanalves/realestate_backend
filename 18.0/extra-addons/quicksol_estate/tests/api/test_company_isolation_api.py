...
def test_01_oauth_token_creation(base_url, client_id, client_secret):
    ...
    if data["token_type"] != "Bearer":
        print(
            f"✗ FAIL: Token type deveria ser 'Bearer', recebido '{data['token_type']}'"
        )
        return False
...
def test_04_login_user_a(base_url, user_a_email, user_a_password):
    ...
    if not result.get("uid"):
        print("✗ FAIL: UID não retornado")
        print(f"Result: {result}")
        return False

    print("✓ PASS: User A autenticado com sucesso")
...
def test_05_login_user_b(base_url, user_b_email, user_b_password):
    ...
    print("✓ PASS: User B autenticado com sucesso")
    print(f"  UID: {result.get('uid')}")
    print(f"  Username: {result.get('username')}")
    print(f"  Session ID: {result.get('session_id', 'N/A')[:20]}...")
    return True
...
def test_06_login_invalid_credentials(base_url):
    ...
    if response.status_code != 200:
        print(
            f"✗ FAIL: Esperado 200 (Odoo retorna 200 mesmo com erro), recebido {response.status_code}"
        )
        return False
    ...
    if "error" not in data and data.get("result", {}).get("uid"):
        print("✗ FAIL: Credenciais inválidas foram aceitas")
        return False

    print("✓ PASS: Credenciais inválidas rejeitadas")
    return True
...
def main():
    ...
    print(
        f"  Client Secret: {'*' * len(client_secret) if client_secret else 'NOT SET'}"
    )
    ...
    results.append(
        test_03_oauth_token_missing_grant_type(base_url, client_id, client_secret)
    )

...
class TestPropertyAPIAuth(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.company = cls.env["res.company"].create(
            {"name": "Test Company Auth", "cnpj": "11.222.333/0001-81"}  # Valid CNPJ
        )
    ...
    def test_02_get_property_with_invalid_token(self):
        ...
        response = self.url_open(
            f"/api/v1/properties/{self.test_property.id}", headers=headers
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_04_get_nonexistent_property(self):
        ...
        response = self.url_open("/api/v1/properties/99999", headers=headers)
    ...
    def test_06_update_property_with_valid_token(self):
        ...
        token = self._get_access_token()
        headers = {"Authorization": f"Bearer {token}"}
        data = {"price": 350000.00, "num_rooms": 3}

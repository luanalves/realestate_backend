...
_CTRL_MODULE = (
    "odoo.addons.quicksol_estate.controllers" ".property_attachments_controller"
)
...
class TestPropertyAttachmentsController(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls._jwt_secret = (
            config.get("oauth_jwt_secret")
            or config.get("admin_passwd")
            or "test-secret"
        )
        cls._oauth_app = (
            cls.env["thedevkitchen.oauth.application"]
            .sudo()
            .create(
                {
                    "name": "F017 Test App",
    ...
    def setUp(self):
        ...
        self._patches = [
            patch(
                "odoo.addons.thedevkitchen_observability.services.tracer.request", _stub
            ),
            ...
            patch(
                "odoo.addons.quicksol_estate.controllers.utils.response.request", _stub
            ),
        ]
    ...
    def _make_upload(
        self, content=b"\xff\xd8\xff\xe0" + b"\x00" * 100, filename="test.jpg"
    ):
    ...
    def test_upload_missing_file_returns_400_missing_file(self):
        ...
        req = self._make_request(
            self.manager, form={"attachment_type": "image"}, files={}
        )
    ...
    def test_upload_file_too_large_returns_413(self):
        ...
        try:
            ...
            self.env["ir.config_parameter"].sudo().set_param(
                "web.max_file_upload_size", "134217728"
            )
    ...
    def test_delete_by_manager_returns_204(self):
        ...
        req = self._make_request(self.manager)
        resp = self._call(
            "delete_attachment", req, property_id=self.property.id, attachment_id=att_id
        )
        self.assertIsInstance(resp, WerkzeugResponse)
    ...
    def test_delete_by_agent_returns_403(self):
        ...
        resp = self._call(
            "delete_attachment", req, property_id=self.property.id, attachment_id=att.id
        )
        self.assertEqual(resp.status, 403)
        self.assertEqual(resp.body["error"], "forbidden")

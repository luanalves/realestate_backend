...
def test_02_oauth_token_invalid_credentials(base_url):
    ...
    try:
        ...
        if response.status_code != 400:
            return print_test_result(
                False, f"Esperado 400, recebido {response.status_code}"
            )

        return print_test_result(
            True, f"Credenciais inválidas rejeitadas (HTTP {response.status_code})"
        )
...
def test_03_oauth_token_missing_grant_type(base_url, client_id, client_secret):
    ...
    try:
        ...
        return print_test_result(
            True, f"Request sem grant_type rejeitado (HTTP {response.status_code})"
        )

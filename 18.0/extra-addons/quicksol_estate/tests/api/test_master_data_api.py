...
class TestMasterDataAPI(HttpCase):
    ...
    def test_list_states_success(self):
        ...
        response = self.url_open(
            "/api/v1/states", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_list_states_filter_by_country(self):
        ...
        response = self.url_open(
            f"/api/v1/states?country_id={brazil_id}",
    ...
    def test_list_location_types_success(self):
        ...
        response = self.url_open(
            "/api/v1/location-types", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_list_property_types_success(self):
        ...
        response = self.url_open(
            "/api/v1/property-types", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_create_property_with_required_fields(self):
        ...
        self.assertEqual(
            response.status_code, 201, "Should create property successfully"
        )
        data = json.loads(response.content.decode("utf-8"))
    ...
    def test_create_property_missing_required_fields(self):
        ...
        self.assertEqual(
            response.status_code, 400, "Should return 400 for missing required fields"
        )
        data = json.loads(response.content.decode("utf-8"))
    ...
    def test_list_owners_success(self):
        ...
        response = self.url_open(
            "/api/v1/owners", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_list_amenities_success(self):
        ...
        response = self.url_open(
            "/api/v1/amenities", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        if len(data) > 0:
            ...
            self.assertIn(
                "Churrasqueira", amenity_names, "Should have 'Churrasqueira' amenity"
            )
    ...
    def test_invalid_bearer_token_format(self):
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                ...
                response = self.url_open(
                    endpoint, headers={"Authorization": "invalid-token-format"}
                )
                self.assertEqual(response.status_code, 401)
                data = json.loads(response.content.decode("utf-8"))
                self.assertIn("error", data)
    ...
    def test_expired_or_invalid_token(self):
        ...
        response = self.url_open(
            "/api/v1/property-types", headers={"Authorization": f"Bearer {fake_token}"}
        )
    ...
    def test_states_with_invalid_country_id(self):
        ...
        response = self.url_open(
            "/api/v1/states?country_id=999999",
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_endpoints_return_reasonable_response_time(self):
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                ...
                self.assertLess(
                    elapsed, 5.0, f"{endpoint} took {elapsed:.2f}s (too slow)"
                )
    ...
    def test_all_master_data_endpoints_return_valid_json(self):
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                ...
                self.assertEqual(
                    response.status_code, 200, f"{endpoint} should return 200"
                )
                data = json.loads(response.content.decode("utf-8"))
                self.assertIsInstance(data, list, f"{endpoint} should return array")

                # Verify each item has an 'id' and 'name'
                if len(data) > 0:
                    for item in data:
                        self.assertIn("id", item, f"{endpoint} items should have 'id'")
                        self.assertIn(
                            "name", item, f"{endpoint} items should have 'name'"
                        )

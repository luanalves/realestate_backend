...
class TestCommissionCalculation(TransactionCase):
    ...
    def test_create_commission_rule(self):
        ...
        self.assertEqual(
            rule.transaction_type, "sale", "Transaction type should be 'sale'"
        )
        self.assertEqual(
            rule.structure_type, "percentage", "Structure should be 'percentage'"
        )
    ...
    def test_calculate_commission_fixed_amount(self):
        ...
        # Assert
        self.assertEqual(
            commission, 10000.00, "Commission should be fixed R$ 10.000,00"
        )
        ...
        self.assertEqual(
            commission_high, 10000.00, "Fixed commission should be same for high value"
        )
    ...
    def test_commission_rule_non_retroactive(self):
        ...
        service = CommissionService(self.env)
        active_rule = service.get_active_rule_for_agent(self.agent_1.id, "sale")
        ...
        self.assertEqual(
            active_rule.percentage, 3.0, "Active rule should have 3% commission"
        )
    ...
    def test_multi_agent_commission_split(self):
        ...
        commission_1 = service.calculate_commission(rule_agent_1, transaction_amount)

        commission_2 = service.calculate_commission(rule_agent_2, transaction_amount)

        # Assert
        self.assertEqual(
            commission_1, 30000.00, "Agent 1 commission should be R$ 30.000,00 (3%)"
        )
        self.assertEqual(
            commission_2, 20000.00, "Agent 2 commission should be R$ 20.000,00 (2%)"
        )
        ...
        self.assertNotEqual(
            rule_agent_1.id, rule_agent_2.id, "Each agent should have separate rule"
        )
        self.assertEqual(
            rule_agent_1.agent_id.id, self.agent_1.id, "Rule 1 should belong to Agent 1"
        )
        self.assertEqual(
            rule_agent_2.agent_id.id, self.agent_2.id, "Rule 2 should belong to Agent 2"
        )
    ...
    def test_company_isolation_commission_rules(self):
        ...
        # Assert
        self.assertEqual(
            len(company_a_rules), 1, "Company A should have exactly 1 rule"
        )
        self.assertEqual(
            company_a_rules[0].id, rule_company_a.id, "Should return Company A's rule"
        )
        self.assertNotIn(
            rule_company_b.id,
            company_a_rules.ids,

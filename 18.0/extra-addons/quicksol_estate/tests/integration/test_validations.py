...
class TestEmailValidations(BaseValidationTest):
    ...
    def test_agent_email_validation_valid(self, mock_email_normalize):
        ...
        for email in self.valid_emails:
            with self.subTest(email=email):
                ...
                agent = self.create_mock_record(
                    "real.estate.agent", {"name": "Test Agent", "email": email}
                )
                ...
                self.assertTrue(
                    validation_passed, f"Valid email {email} should pass validation"
                )
    ...
    def test_tenant_email_regex_validation_valid(self):
        ...
        for email in self.valid_emails:
            with self.subTest(email=email):
                # Arrange
                tenant = self.create_mock_record(
                    "real.estate.tenant", {"name": "Test Tenant", "email": email}
                )
                ...
                # Assert
                self.assertTrue(
                    is_valid, f"Valid email {email} should pass regex validation"
                )
    ...
    def test_tenant_email_regex_validation_invalid(self):
        ...
        for email in self.invalid_emails:
            with self.subTest(email=email):
                ...
                # Assert
                self.assertFalse(
                    is_valid, f"Invalid email {email} should fail regex validation"
                )
    ...
    def test_empty_email_handling(self):
        ...
        for empty_val in empty_values:
            with self.subTest(empty_value=empty_val):
                # Arrange
                agent = self.create_mock_record(
                    "real.estate.agent", {"name": "Test Agent", "email": empty_val}
                )
                tenant = self.create_mock_record(
                    "real.estate.tenant", {"name": "Test Tenant", "email": empty_val}
                )
                ...
                self.assertFalse(
                    should_validate_agent, "Empty agent email should not be validated"
                )
                self.assertFalse(
                    should_validate_tenant, "Empty tenant email should not be validated"
                )
...
class TestCnpjValidations(BaseValidationTest):
        ...
        for raw_cnpj, expected_formatted in self.valid_cnpjs:
            with self.subTest(cnpj=raw_cnpj):
                ...
                self.create_mock_record(
                    "res.company", {"name": "Test Company", "cnpj": raw_cnpj}
                )
    ...
    def test_cnpj_length_validation(self):
        ...
        for cnpj in self.invalid_cnpjs:
            with self.subTest(cnpj=cnpj):
                # Arrange
                self.create_mock_record(
                    "res.company", {"name": "Test Company", "cnpj": cnpj}
                )
                ...
                is_valid = (
                    len(clean_cnpj) == 14
                    and clean_cnpj.isdigit()
                    and clean_cnpj != "00000000000000"  # Not all zeros
                    and clean_cnpj not in ["12345678000100"]
                )  # Known invalid check digits

                # Assert
                self.assertFalse(
                    is_valid, f"Invalid CNPJ {cnpj} should fail validation"
                )
    ...
    def test_empty_cnpj_handling(self):
        ...
        for empty_val in empty_values:
            with self.subTest(empty_value=empty_val):
                # Arrange
                company = self.create_mock_record(
                    "res.company", {"name": "Test Company", "cnpj": empty_val}
                )
                ...
                # Assert
                self.assertFalse(
                    should_validate, "Empty CNPJ should not trigger validation"
                )
...
class TestFieldRequiredValidations(BaseValidationTest):
    ...
    def test_property_required_fields(self):
        ...
        required_data = {"name": "Test Property", "area": 120.5, "condition": "good"}

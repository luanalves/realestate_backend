...
class TestRBACProspector(TransactionCase):
    ...
    def test_prospector_can_read_properties(self):
        ...
        read_property = self.Property.with_user(self.prospector_user).browse(
            property_obj.id
        )
    ...
    def test_prospector_id_not_assigned_for_non_prospector_users(self):
        ...
        manager_user = self.User.create(
            {
                ...
                "groups_id": [
                    (
                        6,
                        0,
                        [self.env.ref("quicksol_estate.group_real_estate_manager").id],
                    )
                ],
                "company_ids": [(6, 0, [self.company_a.id])],
            }
        ...
        self.assertFalse(
            property_obj.prospector_id,
            "prospector_id should be empty for non-Prospector users",
        )

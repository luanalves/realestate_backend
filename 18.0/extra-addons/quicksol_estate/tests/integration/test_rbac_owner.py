...
class TestRBACOwner(TransactionCase):
                ...
                "groups_id": [
                    (
                        ...
                        [
                            ...
                            ).id,  # Allow contact creation
                            cls.env.ref(
                                "base.group_erp_manager"
                            ).id,  # Settings - required to create users
                        ],
                    )
                ],
    ...
    def test_owner_cannot_read_other_company_properties(self):
        ...
        self.assertEqual(
            len(properties), 0, "Owner should not see properties from Company B"
        )
    ...
    def test_owner_can_create_user_in_own_company(self):
        ...
        new_user = (
            self.User.with_user(self.owner_user)
    ...
    def test_owner_cannot_assign_user_to_other_company(self):
        ...
        with self.assertRaises(ValidationError) as ctx:
            self.User.with_user(self.owner_user).with_context(

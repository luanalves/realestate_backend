...
class TestRBACLegal(TransactionCase):
    ...
    def test_legal_can_read_sales(self):
        ...
        sales = self.Sale.with_user(self.legal_user).search([("id", "=", sale.id)])
    ...
    def test_legal_can_read_leases(self):
        ...
        leases = self.Lease.with_user(self.legal_user).search([("id", "=", lease.id)])
    ...
    def test_legal_cannot_modify_sale_price(self):
        ...
        with self.assertRaises(AccessError):
            sale.with_user(self.legal_user).write({"sale_price": 550000.00})
    ...
    def test_legal_cannot_modify_lease_rent(self):
        ...
        with self.assertRaises(AccessError):
            lease.with_user(self.legal_user).write({"rent_amount": 2500.00})
    ...
    def test_legal_cannot_modify_contract_value(self):
        ...
        with self.assertRaises(AccessError):
            sale.with_user(self.legal_user).write({"sale_price": 850000.00})

    def test_legal_cannot_modify_property_details(self):
        """T120: Legal cannot modify property details (negative test)."""
        with self.assertRaises(AccessError):
            self.property_a.with_user(self.legal_user).write({"price": 900000.00})

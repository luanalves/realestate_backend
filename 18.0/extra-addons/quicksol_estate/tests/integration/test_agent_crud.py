    ...
    def test_email_validation(self):
        ...
        for email in invalid_emails:
            with self.assertRaises(
                ValidationError, msg=f"Should reject invalid email: {email}"
            ):
                self.Agent.create(
                    {
                        "name": f"Agent {email}",
...
class TestAgentUpdate(TransactionCase):
    ...
    def test_deactivate_agent_hidden_from_active_list(self):
        ...
        self.assertIn(
            self.agent,
            all_agents,
            "Should include inactive agent with active_test=False",
        )
    ...
    def test_reactivate_agent(self):
        ...
        self.assertFalse(
            self.agent.deactivation_date, "Deactivation date should be cleared"
        )
        self.assertFalse(
            self.agent.deactivation_reason, "Deactivation reason should be cleared"
        )
    ...
    def test_deactivate_already_inactive_agent(self):
        ...
        with self.assertRaises(
            UserError, msg="Should error on deactivating inactive agent"
        ):
            self.agent.action_deactivate()
    ...
    def test_reactivate_already_active_agent(self):
        ...
        with self.assertRaises(
            UserError, msg="Should error on reactivating active agent"
        ):
            self.agent.action_reactivate()
    ...
    def test_audit_logging_on_deactivation(self):
        ...
        self.assertFalse(self.agent.active, "Agent should be inactive")
        self.assertEqual(
            self.agent.deactivation_date,
            fields.Date.today(),
            "Deactivation date should be today",
        )
        self.assertEqual(
            self.agent.deactivation_reason, reason, "Deactivation reason should match"
        )

        # Verify mail.thread messages exist (at least creation message)
        self.assertGreater(
            len(self.agent.message_ids),
            0,
            "Should have message_ids (mail.thread integration)",
        )

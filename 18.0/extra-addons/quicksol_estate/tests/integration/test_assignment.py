...
class TestAgentPropertyAssignment(TransactionCase):
    ...
    def setUp(self):
        ...
        self.company_b = self.env["res.company"].search(
            [("name", "=", "Company B Test")], limit=1
        )
        ...
        self.location_type_urban = self.env["real.estate.location.type"].search(
            [("code", "=", "URB")], limit=1
        )
        ...
        self.property_type_house = self.env["real.estate.property.type"].search(
            [("name", "=", "House")], limit=1
        )
        ...
        self.property_type_apartment = self.env["real.estate.property.type"].search(
            [("name", "=", "Apartment")], limit=1
        )
    ...
    def test_assign_agent_to_property(self):
        ...
        self.assertIsNotNone(
            assignment.assignment_date, "Assignment date should be set"
        )
    ...
    def test_assign_agent_cross_company_forbidden(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should reject cross-company assignment"
        ):
    ...
    def test_multiple_agents_per_property(self):
        ...
        self.assertTrue(
            assignment1.id and assignment2.id, "Both assignments should be created"
        )
        ...
        property_a2 = self.env["real.estate.property"].create(
            {
                ...
                "company_ids": [(6, 0, [self.company_a.id])],
                "property_type_id": self.env["real.estate.property.type"]
                .search([], limit=1)
                .id,
                "location_type_id": self.env["real.estate.location.type"]
                .search([], limit=1)
                .id,
                "state_id": self.env["res.country.state"].search([], limit=1).id,
                "price": 300000.00,

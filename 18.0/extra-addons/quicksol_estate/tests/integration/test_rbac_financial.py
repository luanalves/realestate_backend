...
class TestRBACFinancial(TransactionCase):
    ...
    def test_financial_can_update_commission_rule(self):
        ...
        rule.with_user(self.financial_user).write({"percentage": 7.5})
    ...
    def test_financial_can_read_sales(self):
        ...
        sales = self.Sale.with_user(self.financial_user).search([("id", "=", sale.id)])
    ...
    def test_financial_cannot_edit_sales(self):
        ...
        with self.assertRaises(AccessError):
            sale.with_user(self.financial_user).write({"sale_price": 550000.00})
    ...
    def test_financial_marks_commission_as_paid(self):
        ...
        transaction.with_user(self.financial_user).with_context(
    ...
    def test_financial_generates_commission_report(self):
        ...
        paid_commissions = self.CommissionTransaction.with_user(
            self.financial_user
        ).search(
            [("company_id", "=", self.company_a.id), ("payment_status", "=", "paid")]
        )
    ...
    def test_financial_cannot_edit_properties(self):
        ...
        with self.assertRaises(AccessError):
            self.property_a.with_user(self.financial_user).write({"price": 999999.00})

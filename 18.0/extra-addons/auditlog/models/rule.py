...
class AuditlogRule(models.Model):
    ...
    def _is_model_excluded(self, model_name, excluded_patterns):
        """Check if a model should be excluded based on patterns."""
    ...
    def _auto_create_rules(self):
        ...
        try:
            ...
            # ir.config.parameter not yet available
            _logger.debug("ir.config_parameter not available yet")
            return
        
        # Check if auto-logging is enabled
        auto_log = IrConfigParam.get_param('auditlog.auto_log', default='False')
        if auto_log != 'True':
            return
        
        # Get configuration
        log_read = IrConfigParam.get_param('auditlog.auto_log_read', default='False') == 'True'
        log_write = IrConfigParam.get_param('auditlog.auto_log_write', default='True') == 'True'
        log_create = IrConfigParam.get_param('auditlog.auto_log_create', default='True') == 'True'
        log_unlink = IrConfigParam.get_param('auditlog.auto_log_unlink', default='True') == 'True'
        excluded_patterns_str = IrConfigParam.get_param(
            'auditlog.excluded_models',
            default='ir.%,base.%,mail.%,bus.%,web.%,report.%,auditlog.%'
        )
        excluded_patterns = [p.strip() for p in excluded_patterns_str.split(',')]
        ...
        for model in all_models:
            # Skip if excluded
            if self._is_model_excluded(model.model, excluded_patterns):
                continue
            
            # Skip if rule already exists
            existing_rule = self.sudo().search([('model_id', '=', model.id)], limit=1)
            if existing_rule:
                continue
            ...
            try:
                rule_vals = {
                    'name': f'Auto: {model.name}',
                    'model_id': model.id,
                    'log_type': 'fast',  # Use 'fast' for better performance
                    'log_read': log_read,
                    'log_write': log_write,
                ...
                rule = self.sudo().create(rule_vals)
                rule.subscribe()
                created_count += 1
            except Exception:
                # Skip models that can't be audited
                continue
        
        if created_count > 0:
            _logger.info(f'Auto-created {created_count} audit rules')
    ...
    def _register_hook(self):
        ...
        # Schedule auto-creation after transaction commits
        # This ensures ir.config_parameter is available
        try:
            self.env.cr.postcommit.add(self._try_auto_create_rules)
        except Exception as e:
            _logger.debug("Could not schedule auto-create: %s", e)
    ...
    @api.model
    def _try_auto_create_rules(self):
        """
        Try to auto-create rules after registry is ready.
        Uses @api.model because:
        - Doesn't need existing records
        - Can be called from postcommit
        - More performant (no recordset loading)
        """
        # Need a new cursor for postcommit operations
        with self.env.registry.cursor() as cr:
            env = api.Environment(cr, SUPERUSER_ID, {})
            
            try:
                ICP = env['ir.config_parameter'].sudo()
                auto_log = ICP.get_param('auditlog.auto_log', 'False')
                
                if auto_log == 'True':
                    # Check if already created (prevent duplicates)
                    rules_created = ICP.get_param('auditlog.rules_auto_created', 'False')
                    
                    if rules_created == 'False':
                        _logger.info("🔄 Auto-creating audit rules...")
                        env['auditlog.rule']._auto_create_rules()
                        ICP.set_param('auditlog.rules_auto_created', 'True')
                        cr.commit()  # Commit the changes explicitly
                        _logger.info("✅ Audit rules created successfully")
            except Exception as e:
                _logger.warning("⚠️ Could not auto-create audit rules: %s", e)
                cr.rollback()

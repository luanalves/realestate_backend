...
class TestLoginEndpoint(TransactionCase):
    ...
    def test_login_successful_creates_session(self):
        ...
        self.assertTrue(len(self.test_user.company_ids) > 0)
    ...
    def test_login_failed_no_companies(self):
        ...
        self.assertEqual(len(self.no_company_user.company_ids), 0)
...
class TestProfileUpdateEndpoint(TransactionCase):
    ...
    def test_profile_update_cannot_modify_company(self):
        ...
        original_companies = self.test_user.company_ids
        ...
        self.assertNotIn('company_ids', allowed_fields)
        self.assertEqual(self.test_user.company_ids, original_companies)
...
class TestSessionIsolation(TransactionCase):
    ...
    def test_session_isolation_prevents_privilege_escalation(self):
        ...
        self.assertNotEqual(
            self.user_a.company_ids.ids,
            self.user_b.company_ids.ids
        )
        
        # User A's session should only access User A's companies
        self.assertTrue(self.company1.id in self.user_a.company_ids.ids)
        self.assertFalse(self.company2.id in self.user_a.company_ids.ids)
        
        # User B's session should only access User B's companies
        self.assertTrue(self.company2.id in self.user_b.company_ids.ids)
        self.assertFalse(self.company1.id in self.user_b.company_ids.ids)

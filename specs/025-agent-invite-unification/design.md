...
**New component**: `quicksol_estate/services/agent_service.py`, exposing `create_agent_from_profile(env, profile, agent_payload)`. This follows an existing precedent in the codebase — `thedevkitchen_user_onboarding` already imports directly from `quicksol_estate/services/role_resolver.py` (`quicksol_estate` is already a declared manifest dependency), so a new service module in the same location, using the same cross-module import pattern, is idiomatic here rather than a novel structure.

...
flowchart TD
    ...
    L4["**200 OK**\n\n{\n  'status': 'success',\n  'data': [ { id, name, mimetype, size,\n              attachment_type, download_url,\n              _links: { self, download } } ],\n  'count': 1,\n  'total': 3,\n  'limit': 50,\n  'offset': 0\n}"] --> L5

    L5{total > count?\nHá mais páginas?}
    L5 -->|Sim| L6["**GET .../attachments?limit=50&offset=50**\n→ próxima página"]
    L5 -->|Não| Done([✅ Lista completa obtida\nUsar download_url de cada item para baixar])
```
...
flowchart TD
    ...
    D0{Tenho o download_url?}
    D0 -->|Não| D0b["**GET /api/v1/properties/{id}/attachments**\n→ pegar campo download_url do item desejado\n→ J2 Listagem"]
    D0 -->|Sim| D1
    D0b --> D1

    D1["**GET {download_url}**\nEx: GET /api/v1/properties/17/attachments/42/download\n\nHeaders:\n  Authorization: Bearer {token}\n  X-Session-Id: {session_id}"] --> D1R{Resposta}
...
flowchart TD
    ...
    Del2R -->|403 Forbidden\nAgent tentou excluir| ERR1([❌ Apenas Owner / Manager\npodem excluir\nAgent só pode fazer upload])
...
| `POST .../attachments` | 403 | Agent sem acesso à propriedade |
...
| `DELETE .../attachments/{id}` | 403 | Agent não pode excluir (apenas Owner/Manager) |
...
|---|---|
| **Imagens** | `image/jpeg`, `image/png`, `image/webp`, `image/gif`, `image/tiff`, `image/bmp` |
| **Documentos** | `application/pdf`, `application/msword`, `application/vnd.openxmlformats-officedocument.wordprocessingml.document`, `application/vnd.ms-excel`, `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `application/vnd.ms-powerpoint`, `application/vnd.openxmlformats-officedocument.presentationml.presentation` |

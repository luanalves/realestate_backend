...
**ADR References**: ADR-003, ADR-004, ADR-007, ADR-008, ADR-009, ADR-011, ADR-015, ADR-018, ADR-019, ADR-022
...
**Acceptance Criteria**:
- [ ] `POST /api/v1/properties/{id}/attachments` com `attachment_type=image` e arquivo JPEG/PNG/WebP/GIF aceita e persiste o arquivo
...
|------|-----------|-------------|--------|
| Unit | `test_image_mimetype_allowed()` | image/jpeg, image/png, image/webp, image/gif aceitos | ⚠️ Required |
...
- FR2.3: A resposta é um stream HTTP com `Content-Type` correto, `Content-Disposition: attachment; filename="..."`, `Content-Security-Policy: default-src 'none'` e `X-Content-Type-Options: nosniff`
...
- Streaming de download: binário lido do filestore Odoo e entregue em chunks, sem carregar arquivo inteiro em memória
...
- E2E API tests de isolamento e acesso não autenticado
...
### Backend Validation
- [ ] ADR-011: Triple decorators em todos os 3 endpoints (upload, download, delete)

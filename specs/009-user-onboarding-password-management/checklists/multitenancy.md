...
- [x] CHK004 O comportamento esperado para conflito de `document` em tenant existente sem vínculo de `res.users` está completamente definido (mensagem, código, fluxo de resolução)? [Completeness, Spec §Clarifications Q5] — Definido: conflito por empresa ativa (409), mensagem explícita e resolução manual quando tenant sem `res.users`.

## Requirement Clarity

- [x] CHK005 O termo “isolamento multi-tenancy” está operacionalizado com critérios verificáveis (filtros, escopo de busca, respostas esperadas) e não apenas textual? [Clarity, Spec §User Story 1, Spec §User Story 4] — Definido por regras operacionais (`X-Company-ID`, 404 genérico, escopo por empresa ativa, precedência de erro).
...
- [x] CHK008 O requisito para `profile=portal` explicita com clareza quais campos condicionais são obrigatórios e em quais cenários exatos de validação? [Clarity, Spec §FR1.11, Spec §Clarifications Q2] — Definido: campos obrigatórios condicionais apenas no `POST /users/invite` para `profile=portal`.

## Requirement Consistency

- [x] CHK009 As regras de isolamento descritas nas User Stories são consistentes com os FRs (especialmente FR1/FR3/US4/US5)? [Consistency, Spec §FR1, Spec §FR3, Spec §User Story 4, Spec §User Story 5] — Alinhado via critérios adicionados em US1/US4 e FR1/FR5/FR8.
- [x] CHK010 A matriz de autorização por perfil (Owner/Manager/Agent etc.) não conflita com as restrições de isolamento por empresa em nenhum cenário de convite/reenvio? [Consistency, Spec §User Story 1, Spec §User Story 4] — Resolvido com regra de precedência AuthZ(403) > Isolamento(404) e escopo da empresa ativa.
...
- [x] CHK012 O comportamento de dual record (`res.users` + `real.estate.tenant`) está consistente entre critérios de criação, validação e conflitos de documento? [Consistency, Spec §FR1.11, Spec §Clarifications Q2, Q5] — Consistência formalizada para validação condicional e conflito de `document` por empresa.

## Acceptance Criteria Quality

- [x] CHK013 Cada critério de aceitação relevante para isolamento inclui resultado mensurável (status code, escopo de dados, condição de sucesso/falha)? [Acceptance Criteria, Spec §User Story 1, Spec §User Story 4] — ACs de US1/US4 atualizados com resultados explícitos (403/404/409 e escopo por empresa ativa).
...
- [x] CHK016 Os requisitos cobrem cenário primário e alternativo para convite em multiempresa (empresa correta, empresa incorreta, usuário sem acesso)? [Coverage, Spec §User Story 1, Spec §User Story 4] — Coberto em ACs com cenários de empresa correta/incorreta e autorização insuficiente.
- [x] CHK017 Cenários de exceção para token em contexto multiempresa (token válido, expirado, usado, inexistente, inválido) estão completos e sem lacunas? [Coverage, Spec §FR2, Spec §FR3] — Completo com FR2.10 + regras de invalidação concorrente FR3.8/FR4.8.
- [x] CHK018 O spec cobre cenários de recuperação (reenvio de convite, geração de novo reset token, invalidação de anteriores) com condições claras de rollback lógico? [Recovery Flow, Spec §User Story 3, Spec §User Story 4] — Definido: último token vence e anteriores inválidos em forgot/resend.
...
- [x] CHK020 Estão definidos requisitos para comportamento quando `company_id` informado é inválido, ausente ou incompatível com contexto do usuário? [Edge Case, Spec §FR1, Spec §User Story 1] — Definido: 404 genérico para contexto de empresa ausente/inválido/incompatível.
...
- [x] CHK022 Há definição para conflitos de dados entre tenant pré-existente e novo convite portal em empresas diferentes? [Edge Case, Spec §Clarifications Q5] — Definido: conflito de `document` não é global; duplicidade em empresa diferente é permitida.
- [x] CHK023 O comportamento com usuário inativo em fluxos públicos (forgot/reset) está completo e consistente com anti-enumeration? [Edge Case, Spec §User Story 3] — Formalizado em FR3.7 com paridade de status/shape e sem revelar motivo.

## Non-Functional Requirements

- [x] CHK024 Requisitos de segurança para isolamento de dados estão descritos com critérios auditáveis (não apenas intenção arquitetural)? [NFR-Security, Spec §FR1, Spec §FR3] — NFR1 atualizado com campos mínimos auditáveis (`correlation_id`, `active_company_id`, `reason_code`).
- [x] CHK025 Requisitos de desempenho para operações de token e validação multiempresa estão quantificados e testáveis? [Gap, NFR-Performance, Plan §Technical Context] — Decisão formalizada: sem meta numérica fixa neste ciclo; monitoramento obrigatório e vedação de regressão relevante no mesmo ambiente.
...
- [x] CHK032 O documento resolve potenciais conflitos entre “mesma resposta por segurança” e necessidades de troubleshooting operacional? [Conflict, Spec §FR3.1] — Resolvido: resposta externa genérica + observabilidade técnica interna sem exposição de dados.

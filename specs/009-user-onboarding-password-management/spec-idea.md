...
**Created**: 2026-02-16
**Status**: Draft
**ADR References**: ADR-003, ADR-004, ADR-007, ADR-008, ADR-009, ADR-011, ADR-015, ADR-017, ADR-018, ADR-019, ADR-022
...
**Acceptance Criteria**:
- [ ] Given Owner/Manager/Agent autenticado, when cria usuário via `POST /api/v1/users/invite` com `name`, `email`, `cpf` e `profile`, then usuário é criado como `res.users` SEM senha (bloqueado para login) e email de convite é enviado
...
### User Story 4: Owner/Manager Reenvia Convite (Priority: P2)
...
**IMPORTANTE**: O endpoint de login existente (`POST /api/v1/users/login`) em `user_auth_controller.py` **NÃO deve ser modificado**. Todos os perfis utilizam o mesmo fluxo de login já implementado. O login funciona nativamente para qualquer `res.users` do Odoo com senha definida — basta que o novo perfil siga o padrão de cadastro do Owner (criação como `res.users` com grupo atribuído).
...
- FR1.8: CPF duplicado retorna 409 Conflict
...
| 409 | CPF already exists | `{"error": "conflict", "field": "cpf"}` |
| 500 | Email sending failed | `{"error": "email_error", "message": "Failed to send invite email"}` |
...
| Profile Value | Odoo Group XML ID |
|---------------|-------------------|
...
**Dependencies**:
- Existing modules: `thedevkitchen_apigateway` (auth, middleware, sessions), `quicksol_estate` (groups, models)
...
- `POST /api/v1/users/invite` (com authorization matrix)
...
2. **Postman Collection** (per ADR-016)
   ...
   - Include invite flow, set-password, forgot-password, reset-password
   - Test scripts for token extraction

3. **Constitution Update** (MANDATORY — new patterns introduced)
   - Location: `.specify/memory/constitution.md`
   - Add token hashing pattern, anti-enumeration, email templates

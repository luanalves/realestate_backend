...
- [X] T001 Create module directory `18.0/extra-addons/thedevkitchen_estate_goals/` with `__init__.py`
- [X] T002 Create `18.0/extra-addons/thedevkitchen_estate_goals/__manifest__.py` declaring name, version, author, depends (`quicksol_estate`, `thedevkitchen_apigateway`), data files, installable
- [X] T003 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/models/__init__.py`
- [X] T004 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/controllers/__init__.py`
- [X] T005 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/services/__init__.py`
- [X] T006 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/tests/__init__.py` and `tests/unit/__init__.py`
...
- [X] T007 Implement `thedevkitchen.estate.goal` model in `18.0/extra-addons/thedevkitchen_estate_goals/models/estate_goal.py`:
...
- [X] T008 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/security/ir.model.access.csv` with read ACL for `base.group_user` and full ACL for `base.group_system`

- [X] T009 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/security/record_rules.xml`:
  - Rule 1 (all users): company isolation `[('company_id', '=', user.company_id.id)]`
  - Rule 2 (agent group): own-only restriction `[('user_id', '=', user.id)]`

- [X] T010 [P] Create `18.0/extra-addons/thedevkitchen_estate_goals/data/api_endpoints_data.xml` seeding all 5 endpoints in `thedevkitchen_api_endpoint` table:
...
- [X] T011 Update `18.0/extra-addons/thedevkitchen_estate_goals/__manifest__.py` to include `security/ir.model.access.csv`, `security/record_rules.xml`, `data/api_endpoints_data.xml` in the `data` list
...
- [X] T012 [US1] Create `18.0/extra-addons/thedevkitchen_estate_goals/controllers/goals_controller.py` with class `GoalsController(http.Controller)` and stub routes for all 5 endpoints decorated with `@require_jwt`, `@require_session`, `@require_company`

- [X] T013 [US1] Implement `POST /api/v1/goals` in `goals_controller.py`:
...
- [X] T014 [US1] Implement `PUT /api/v1/goals/<int:goal_id>` in `goals_controller.py`:
...
- [X] T015 [US1] Implement `DELETE /api/v1/goals/<int:goal_id>` in `goals_controller.py`:
...
- [X] T016 [US1] Implement `GET /api/v1/goals` in `goals_controller.py`:
...
- [X] T017 [US1] Write unit tests in `18.0/extra-addons/thedevkitchen_estate_goals/tests/unit/test_estate_goal.py`:
...
- [X] T018 [US1] Write integration test `integration_tests/test_us019_s1_create_goals.sh`:
...
- [X] T019 [US1] Write integration test `integration_tests/test_us019_s2_goal_lifecycle.sh`:
...
- [X] T020 [US2] Create `18.0/extra-addons/thedevkitchen_estate_goals/services/goals_report_service.py` with class `GoalsReportService` and method `compute_report(env, company_id, user_ids, date_from, date_to, operation_type)` returning a dict
...
- [X] T021 [US2] Implement `_query_captacao(env, company_id, user_ids, date_from, date_to, operation_type)` in `goals_report_service.py`:
...
- [X] T022 [US2] Implement `_query_novos_clientes(env, company_id, user_ids, date_from, date_to, operation_type)` in `goals_report_service.py`:
...
- [X] T023 [US2] Implement `_query_visitas(env, company_id, user_ids, date_from, date_to, operation_type)` in `goals_report_service.py`:
...
- [X] T024 [US2] Implement `_query_propostas(env, company_id, user_ids, date_from, date_to, operation_type)` in `goals_report_service.py`:
...
- [X] T025 [US2] Implement `_query_fechamento(env, company_id, user_ids, date_from, date_to, operation_type)` in `goals_report_service.py`:
...
- [X] T026 [US2] Implement `_resolve_period(year, month, date_from, date_to)` helper in `goals_report_service.py`:
...
- [X] T027 [US2] Implement `_compute_user_row(user_id, goals, achievements)` in `goals_report_service.py`:
...
- [X] T028 [US2] Implement `GET /api/v1/goals/report` endpoint in `goals_controller.py`:
...
- [X] T029 [US2] Write unit tests in `test_estate_goal.py`:
...
- [X] T047 [P] Write unit tests for all 5 SQL query methods in `test_estate_goal.py`:
...
- [X] T048 [P] Create seed data script `integration_tests/seeds/019_goals_seed.py` (create `integration_tests/seeds/` directory if absent):
...
- [X] T030 [US2] Write integration test `integration_tests/test_us019_s3_report_single_month.sh`:
...
- [X] T031 [US3] Implement `_compute_totals(user_rows)` in `goals_report_service.py`:
...
- [X] T032 [US3] Implement `operation_type` exclusion logic in `compute_report()`:
...
- [X] T033 [US3] Implement accumulated period mode in `compute_report()`:
...
- [X] T034 [US3] Extend `GET /api/v1/goals/report` in `goals_controller.py` with manager-scope user resolution:
...
- [X] T045 [US3] Implement `profile` filter in `goals_report_service.py` and `goals_controller.py`:
...
- [X] T035 [US3] Write integration test `integration_tests/test_us019_s4_report_date_range.sh`:
...
- [X] T036 [US3] Write integration test `integration_tests/test_us019_s5_rbac_matrix.sh`:
...
- [X] T037 [US4] Create `18.0/extra-addons/thedevkitchen_estate_goals/views/estate_goal_views.xml`:

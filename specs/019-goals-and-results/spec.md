...
**Created**: 2026-05-11
**Status**: Draft
**Input**: Metas e Resultados — módulo para definir e acompanhar metas mensais individuais por usuário em 5 métricas do funil imobiliário (captações, novos clientes, visitas, propostas, fechamento), com cálculo automático de conquistas via entidades existentes e relatório comparativo via API headless e UI Odoo
...
- Q: What is the actual `agent_id` field type on `real.estate.service`? → A: **Corrected by Phase 0 research**: `Many2one(res.users)` — direct FK to res.users (NOT a two-hop via `real.estate.agent`). Only `real.estate.property` and `real.estate.proposal` use the two-hop `real.estate.agent` intermediary.
- Q: Should a user be `complete` if they meet 100% of goals they have, even if other metrics have no goal set? → A: Yes — `complete` = all *set* goals met; metrics with no goal (null) are neutral and excluded from evaluation
- Q: When `date_from`/`date_to` are provided, is `year` still required? → A: No — `year` is required only in single-month mode; it is optional and ignored when `date_from`/`date_to` are provided
- Q: When the report is filtered by `operation_type=sale` or `rent`, how should goals set with `operation_type=all` be treated? → A: Excluded — only goals explicitly set as `sale` or `rent` appear in filtered mode; `all` goals are never matched against a filtered conquista to avoid misleading `completion_pct`
- Q: Should the report endpoint support pagination, or return all users in one response? → A: No pagination — all matching users returned in a single response; hard cap of 200 users per request (422 Unprocessable Entity if exceeded)
...
| Novos Clientes | `real.estate.service` | `agent_id` (direct `res.users`) | `create_date` | `operation_type` |
...
> Technology-agnostic results that validate the feature delivers its intended business value.

- **SC-001**: Um gestor consegue definir a meta mensal de um corretor para qualquer uma das 5 métricas em menos de 1 minuto
- **SC-002**: O relatório de metas vs. conquistas de um mês carrega em menos de 500ms para equipes de até 50 usuários
- **SC-003**: O relatório acumulado (período de 12 meses) para uma equipe de até 50 usuários carrega em menos de 2 segundos
- **SC-004**: O corretor visualiza seus próprios resultados sem depender do gestor — acesso direto via app (sem relatório manual intermediário)
- **SC-005**: 100% das conquistas são calculadas automaticamente — zero entrada manual de dados de resultado
- **SC-006**: Dados de metas entre imobiliárias distintas não se cruzam — isolamento multi-tenant verificado em cenários de teste
- **SC-007**: Tentativa de acesso de um corretor aos dados de outro corretor retorna erro sem expor qualquer dado

### Implementation Checklist

> Verificação de completude técnica — usada como critério de aceite na revisão de código e QA.

#### Backend
- [ ] Goal CRUD (POST/PUT/DELETE/GET) implementado e testado
- [ ] Cálculo de conquistas correto para todas as 5 métricas e 3 tipos de operação
- [ ] VGV correto para captacao/propostas/fechamento
- [ ] Relatório com todos os 5 filtros funcionando
- [ ] Linha de totais correta no relatório
- [ ] Período acumulado (date_from/date_to) testado
- [ ] Isolamento multi-company verificado
- [ ] RBAC: corretores veem apenas os próprios dados; gestores/proprietários veem toda a equipe

#### Frontend (Odoo Admin UI)
- [ ] Menu "Imobiliária > Metas" abre sem erros JavaScript
- [ ] Nenhum atributo `groups` em `<menuitem>`
- [ ] Testes Cypress E2E para list e form views passando
- [ ] Console do browser sem erros após navegação manual

#### Seeds
- [ ] Arquivo de seed criado com prefixo `seed_019_` em todos os logins
- [ ] Cobre Owner, Director, Manager, Agent + empresa B (isolamento)
- [ ] Cobre todas as 5 métricas e os 3 tipos de operação
- [ ] Seed é idempotente (seguro para executar múltiplas vezes)

#### Documentação (Pós-desenvolvimento)
- [ ] Swagger/OpenAPI gerado via `thedevkitchen.swagger` (ADR-005)
- [ ] Coleção Postman gerada via `thedevkitchen.postman` (ADR-016)
- [ ] Constituição atualizada com padrão Report Endpoint (v1.8.0)
...
- `real.estate.service.agent_id` is a **direct** `Many2one(res.users)` (no intermediary)
- `real.estate.property.agent_id` and `real.estate.proposal.agent_id` use two-hop `Many2one(real.estate.agent)` → `.user_id` for attribution

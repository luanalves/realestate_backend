...
- [ ] T022 [P] [US1] API test `18.0/extra-addons/quicksol_estate/tests/api/test_service_endpoints.py` — covers POST/GET/PUT/DELETE/PATCH stage on `/api/v1/services`, HATEOAS links presence, error codes 400/401/403/404/409/422/423
- [ ] T023 [P] [US1] Integration shell test `integration_tests/test_us15_s1_agent_creates_service_lifecycle.sh` — full lifecycle: auth → POST → walk all stages → verify audit timeline (uses seeded data)
...
- [ ] T028 [US1] Update `18.0/extra-addons/quicksol_estate/controllers/__init__.py` to import `service_controller`
- [ ] T029 [US1] Update `18.0/extra-addons/quicksol_estate/services/__init__.py` (create if missing) to import `partner_dedup_service` and `service_pipeline_service`
- [ ] T030 [US1] Register new endpoints in Swagger via DB (`thedevkitchen_api_endpoint`) — add data XML entries for the 6 US1 endpoints per ADR-005 / skill swagger-updater
- [ ] T031 [US1] Run T018–T023 and confirm GREEN
